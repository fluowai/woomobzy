...
async function runMigration() {
  try {
    ...
    if (resellerError || !reseller) {
      ...
      throw new Error("Revenda Delazari Imóveis não encontrada. Certifique-se de que ela existe.");
    }
    ...
    for (const url of urls) {
      ...
      try {
        ...
        const propertyData = await page.evaluate(() => {
           ...
           let price = '';
           const priceEls = Array.from(document.querySelectorAll('*')).filter(el => el.innerText && el.innerText.includes('R$'));
           if (priceEls.length) {
               // heuristic: find the one with largest font size or just the first prominent one
               price = priceEls.find(el => el.tagName === 'H2' || el.tagName === 'H3' || el.tagName === 'P')?.innerText || priceEls[0].innerText;
               // cleanup
        ...
        if (insertError) {
            console.error(`Erro ao inserir imóvel ${url} no BD:`, insertError.message);
        } else {
