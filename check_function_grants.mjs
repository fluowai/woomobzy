...
async function run() {
  try {
    ...
    if (!rows[0].has_exec) {
      ...
      await pool.query(`GRANT EXECUTE ON FUNCTION public.get_auth_organization_id() TO authenticated;`);
      await pool.query(`GRANT EXECUTE ON FUNCTION public.get_auth_organization_id() TO anon;`);
      console.log('Granted.');
    }
  } catch (err) {
