...
const FeaturesBlockSettings: React.FC<FeaturesBlockSettingsProps> = ({
  ...
  const handleAddFeature = () => {
    const newFeatures = [
      ...(config.features || []),
      { title: 'Nova Funcionalidade', description: 'Descrição da funcionalidade', icon: '✨' },
    ];
    handleChange('features', newFeatures);
  };

  const handleUpdateFeature = (index: number, key: keyof Feature, value: string) => {
      ...
      <div>
        ...
        <div className="space-y-4">
          {(config.features || []).map((feature, index) => (
            <div key={index} className="p-4 border border-gray-200 rounded-lg bg-gray-50 space-y-3 relative group">
              <button
