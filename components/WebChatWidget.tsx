...
export const WebChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<{role: 'agent'|'user', content: string}[]>([
    { role: 'agent', content: 'Olá! Sou o assistente virtual. Como posso te ajudar hoje?' }
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'agent', content: response.reply || 'Recebido.' }]);
    } catch (error) {
      setMessages(prev => [...prev, { role: 'agent', content: 'Desculpe, estou com dificuldades de conexão no momento.' }]);
    } finally {
      setLoading(false);
    }
  };
      ...
      {isOpen && (
        <div className="mb-4 w-[350px] h-[500px] bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col animate-in slide-in-from-bottom-5 fade-in duration-300">
          <div className="bg-emerald-600 text-white p-4 flex items-center justify-between">
            ...
            <button onClick={toggleChat} className="text-emerald-100 hover:text-white transition">
              <X size={20} />
            </button>
          </div>
