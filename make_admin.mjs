...
async function run() {
  ...
  const { data: profileObj, error: profileErr } = await supabase.from('profiles').select('*').limit(1);
  if (!profileErr) {
  ...
  const { data: userObj, error: userErr } = await supabase.from('User').select('*').limit(1);
  if (!userErr) {
