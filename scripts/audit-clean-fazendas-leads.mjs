...
const JSON_PATH = path.join(OUTPUT_DIR, APPLY ? "fazendas-leads-cleanup-report.applied.json" : "fazendas-leads-cleanup-report.dry-run.json");
const MD_PATH = path.join(OUTPUT_DIR, APPLY ? "RELATORIO_LIMPEZA_LEADS_FAZENDAS_BRASIL_APLICADO.md" : "RELATORIO_LIMPEZA_LEADS_FAZENDAS_BRASIL_DRY_RUN.md");
...
function hasSharedContact(a, b) {
  ...
  return Boolean((phoneA && phoneA === phoneB) || (emailA && emailA === emailB));
}

function isSafeDuplicateGroup(leads) {
  if (leads.length < 2) return false;
  const phones = new Set(leads.map((lead) => normalizePhone(lead.phone)).filter(Boolean));
  const emails = new Set(leads.map((lead) => normalizeEmail(lead.email)).filter(Boolean));
  if (phones.size === 1 && phones.size > 0) return true;
  if (emails.size === 1 && emails.size > 0) return true;
  return leads.some((lead, index) => leads.slice(index + 1).some((other) => hasSharedContact(lead, other)));
}
...
async function updateOptionalTable(table, duplicateId, keeperId, organizationId) {
  ...
  if (optionalCodes.has(error.code)) return { table, ok: false, skipped: true, reason: error.message };
  throw error;
}
...
async function dedupeGroup({ organizationId, keeper, duplicates }) {
  ...
  const merged = {
    ...
    last_contacted_at: [keeper.last_contacted_at, ...duplicates.map((lead) => lead.last_contacted_at)].filter(Boolean).sort().at(-1) || keeper.last_contacted_at || null,
    next_follow_up_at: keeper.next_follow_up_at || duplicates.find((lead) => lead.next_follow_up_at)?.next_follow_up_at || null,
    chat_jid: keeper.chat_jid || duplicates.find((lead) => lead.chat_jid)?.chat_jid || null,
    classification: keeper.classification || duplicates.find((lead) => lead.classification)?.classification || null,
    source: keeper.source || duplicates.find((lead) => lead.source)?.source || null,
  };

  const result = { keeperId: keeper.id, deletedIds: duplicateIds, relationUpdates: [] };
  ...
  for (const duplicateId of duplicateIds) {
    result.relationUpdates.push(await updateOptionalTable("lead_activities", duplicateId, keeper.id, organizationId));
    result.relationUpdates.push(await updateOptionalTable("lead_followups", duplicateId, keeper.id, organizationId));
    result.relationUpdates.push(await updateOptionalTable("emails", duplicateId, keeper.id, organizationId));
    result.relationUpdates.push(await updateOptionalTable("quiz_submissions", duplicateId, keeper.id, organizationId));
    ...
    const tagRows = (duplicateTags || []).map((row) => ({ organization_id: organizationId, lead_id: keeper.id, tag: row.tag }));
    if (tagRows.length) {
      const { error: tagUpsertError } = await supabase.from("lead_tags").upsert(tagRows, { onConflict: "lead_id,tag" });
...
function toMarkdown(report) {
  const duplicateRows = report.duplicateNameGroups
    .slice(0, 30)
    .map((group) => `| ${csvValue(group.name)} | ${group.count} | ${group.safeToMerge ? "sim" : "revisar"} | ${csvValue(group.keeper?.name)} | ${group.duplicatesToDelete.length} |`)
    .join("\n") || "| Nenhum | 0 | - | - | 0 |";

  const partnerRows = report.partnerCandidates
    .slice(0, 40)
    .map((lead) => `| ${csvValue(lead.name)} | ${csvValue(lead.phone)} | ${csvValue(lead.email)} | ${csvValue(lead.source)} | ${lead.signals.join(", ")} |`)
    .join("\n") || "| Nenhum | - | - | - | - |";
...
const leads = await fetchAll(
  ...
  (query) => query.eq("organization_id", organization.id).order("created_at", { ascending: true }),
);
...
for (const row of activities) activityCount.set(row.lead_id, (activityCount.get(row.lead_id) || 0) + 1);
const followupCount = new Map();
for (const row of followups) followupCount.set(row.lead_id, (followupCount.get(row.lead_id) || 0) + 1);
...
for (const lead of enriched) {
  ...
  byName.set(lead.normalized_name, [...(byName.get(lead.normalized_name) || []), lead]);
}
...
const duplicateNameGroups = [...byName.entries()]
  ...
  .map(([name, group]) => {
    ...
    return {
      ...
      safeToMerge,
      keeper,
      duplicatesToDelete: safeToMerge ? group.filter((lead) => lead.id !== keeper.id) : [],
      leads: group.sort((a, b) => String(a.created_at || "").localeCompare(String(b.created_at || ""))),
    };
  })
  .sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));

const partnerCandidates = enriched
  .map((lead) => ({ ...lead, signals: partnerSignals(lead, tagMap.get(lead.id) || []) }))
  .filter((lead) => lead.signals.length > 0)
  .sort((a, b) => b.signals.length - a.signals.length || String(b.created_at || "").localeCompare(String(a.created_at || "")));

const applied = [];
if (APPLY) {
  for (const group of duplicateNameGroups.filter((item) => item.safeToMerge && item.duplicatesToDelete.length > 0)) {
    applied.push(await dedupeGroup({
...
const totalLeadsAfter = APPLY
  ? (await fetchAll("leads", "id", (query) => query.eq("organization_id", organization.id))).length
  : leads.length - duplicateNameGroups.reduce((sum, group) => sum + group.duplicatesToDelete.length, 0);
...
const report = {
  ...
  totalLeadsAfter,
  duplicateNameGroups,
  duplicateLeadRows: duplicateNameGroups.reduce((sum, group) => sum + group.count, 0),
  safeDuplicateGroups: duplicateNameGroups.filter((group) => group.safeToMerge).length,
  deletedDuplicateLeads: APPLY ? applied.reduce((sum, item) => sum + item.deletedIds.length, 0) : duplicateNameGroups.reduce((sum, group) => sum + group.duplicatesToDelete.length, 0),
  applied,
