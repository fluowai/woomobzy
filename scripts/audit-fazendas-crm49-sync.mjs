...
const { data: orgs, error: orgError } = await supabase
  ...
  .or(
    'slug.eq.fazendas-brasil,slug.eq.fazendasbrasil,name.ilike.%Fazendas Brasil%,custom_domain.ilike.%fazendasbrasil.com.br%,custom_domain.ilike.%fazendasbrasil.com%'
  );
...
const attendanceLeadIds = [
  ...new Set(
    taggedRows
      .filter((row) => row.tag === 'atendimento-crm49')
      .map((row) => row.lead_id)
  ),
];
...
for (const lead of importedLeads || []) {
  countsByStatus[lead.status || 'Sem status'] =
    (countsByStatus[lead.status || 'Sem status'] || 0) + 1;
}
...
const report = {
  ...
  attendanceActivityCount: (activities || []).filter((activity) =>
    activity.description.includes('Atendimento CRM49 #')
  ).length,
  tags: Object.fromEntries(
    ['lista-fazendas-brasil', 'crm49', 'atendimento-crm49'].map((tag) => [
      tag,
      new Set(
        taggedRows.filter((row) => row.tag === tag).map((row) => row.lead_id)
      ).size,
    ])
  ),
  countsByStatus,
...
await fs.writeFile(
  path.join(outputDir, 'fazendas-crm49-audit-report.json'),
