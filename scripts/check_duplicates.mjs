...
async function run() {
  ...
  const duplicates = Array.from(byTitle.entries()).filter(
  ...
  if (duplicates.length > 0) {
    ...
    duplicates.slice(0, 10).forEach(([title, items]) => {
      ...
      items.forEach((item) => {
        console.log(
          `  ID: ${item.id} | Price: ${item.price} | Status: ${item.status} | Images: ${item.images?.length || 0}`
        );
      });
    });
  }
  ...
  const exactDuplicates = Array.from(byTitlePrice.entries()).filter(
    ([_, items]) => items.length > 1
  );
  console.log(
