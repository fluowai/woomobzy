...
async function run() {
  ...
  try {
    ...
    orgs.rows.forEach(org => {
      ...
      console.log(`${org.name} | ${org.slug} | Reseller: ${isReseller} | Parent: ${org.parent_id || 'nenhum'} | Status: ${org.status}`);
    });
    ...
    if (wrongInPamas.rows.length === 0) {
      console.log('Nenhum imóvel do Mega Investimentos encontrado na Pamasimóveis.');
    } else {
    ...
    if (wrongInMega.rows.length === 0) {
      console.log('Nenhum imóvel da Pamasimóveis encontrado no Mega Investimentos.');
    } else {
    ...
    pamasProps.rows.forEach((row, i) => {
      console.log(`  ${i+1}. [${row.id}] ${row.title} | R$ ${row.price} | ${row.status} | ${row.purpose} | ${row.img_count || 0} imgs | ${row.external_id || 'sem external_id'}`);
    });
    ...
    hierarchy.rows.forEach(row => {
      ...
      console.log(`${row.revenda} → ${row.cliente || '(nenhum)'}: ${total} imóveis`);
    });
