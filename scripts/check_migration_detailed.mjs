...
async function run() {
  ...
  try {
    ...
    pages.rows.forEach((page) => {
      ...
      console.log(
        `[${org}] ${page.name} | ${page.slug} | ${page.status} | active: ${page.is_active}`
      );
      console.log(
        `  property_selection: ${JSON.stringify(page.property_selection).substring(0, 200)}`
      );
    });
    ...
    withImages.rows.forEach((row) => {
      ...
      console.log(
        `${org}: ${row.total} total | ${row.com_imagens} com imagens | ${row.sem_imagens} sem imagens`
      );
    });
    ...
    `,
      [
        PAMAS_ID,
        'https://megainvestimoveis.com.br/%',
        MEGA_ID,
        'https://pamasimoveis.com.br/%',
      ]
    );
    ...
    pamasSample.rows.forEach((row) => {
      console.log(
        `  ${row.title.substring(0, 60)} | external: ${row.external_id?.substring(0, 80)} | imgs: ${row.img_count || 0}`
      );
    });
    ...
    storage.rows.forEach((row) => {
      ...
      console.log(
        `${org}: ${row.total_objs} objetos storage | ${row.property_objs} ligados a properties`
      );
    });
  } catch (err) {
