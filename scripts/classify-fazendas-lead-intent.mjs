...
const JSON_PATH = path.join(
  OUTPUT_DIR,
  APPLY
    ? 'fazendas-lead-intent.applied.json'
    : 'fazendas-lead-intent.dry-run.json'
);
const MD_PATH = path.join(
  OUTPUT_DIR,
  APPLY
    ? 'RELATORIO_INTENCAO_LEADS_APLICADO.md'
    : 'RELATORIO_INTENCAO_LEADS_DRY_RUN.md'
);
...
const farmContextPatterns = [
  { label: 'fazenda', pattern: /\bfazenda(s)?\b/ },
  {
    label: 'sitio/chacara',
    pattern:
      /\b(sitio|sitios|chacara|chacaras|area rural|imovel rural|propriedade rural)\b/,
  },
  {
    label: 'hectares/alqueires',
    pattern: /\b(hectare|hectares|ha|alqueire|alqueires|alq)\b/,
  },
  {
    label: 'agro',
    pattern: /\b(pecuaria|gado|soja|milho|pastagem|terra|rural)\b/,
  },
];
...
const sellerPatterns = [
  {
    ...
    weight: 3,
    pattern:
      /\b(quero|preciso|gostaria|pretendo|tenho interesse em)\s+(vender|anunciar|negociar)\b/,
  },
  {
    label: 'vendo/vende-se',
    weight: 3,
    pattern: /\b(vendo|vende se|vende-se|estou vendendo|estamos vendendo)\b/,
  },
  {
    label: 'tenho propriedade',
    weight: 2,
    pattern:
      /\b(tenho|possuo|sou proprietario|sou proprietaria|meu pai tem|minha familia tem)\b.{0,80}\b(fazenda|sitio|chacara|area|terra|propriedade)\b/,
  },
  {
    label: 'avaliacao/captacao',
    weight: 2,
    pattern:
      /\b(avaliar|avaliacao|captar|captacao|colocar a venda|por a venda|por para vender|anunciar)\b/,
  },
  {
    label: 'fazenda a venda',
    weight: 3,
    pattern:
      /\b(fazenda|sitio|chacara|area|terra|propriedade)\b.{0,60}\b(a venda|para venda|vendendo|vende)\b/,
  },
  {
    label: 'proprietario',
    weight: 1,
    pattern: /\b(proprietario|proprietaria|dono|dona)\b/,
  },
];
...
const buyerPatterns = [
  {
    ...
    weight: 3,
    pattern:
      /\b(quero|preciso|gostaria|pretendo|tenho interesse em)\s+(comprar|adquirir|investir)\b/,
  },
  {
    label: 'procura/busca',
    weight: 3,
    pattern:
      /\b(procuro|busco|estou procurando|estou buscando|tem alguma|tem fazenda|tem area)\b/,
  },
  {
    label: 'compra fazenda',
    weight: 3,
    pattern:
      /\b(comprar|adquirir|investir em)\b.{0,80}\b(fazenda|sitio|chacara|area|terra|propriedade)\b/,
  },
  {
    label: 'pergunta preco rural',
    weight: 4,
    pattern:
      /\b(qual|quanto|saber)\b.{0,40}\b(preco|valor|pedida)\b.{0,80}\b(fazenda|hectare|ha|alqueire|terra)\b/,
  },
  {
    label: 'preco fazenda',
    weight: 3,
    pattern:
      /\b(preco|valor|pedida)\b.{0,60}\b(fazenda|hectare|ha|alqueire|terra)\b/,
  },
  {
    label: 'orcamento',
    weight: 2,
    pattern:
      /\b(orcamento|budget|ate r\$|tenho r\$|valor ate|faixa de valor)\b/,
  },
  {
    label: 'visita/disponibilidade',
    weight: 1,
    pattern:
      /\b(visitar|visita|disponivel|disponibilidade|mais informacoes|preco|valor)\b/,
  },
  {
    label: 'financiamento/investidor',
    weight: 1,
    pattern: /\b(financiamento|credito rural|investidor|investimento)\b/,
  },
];
...
const partnerPatterns = [
  { label: 'creci', weight: 3, pattern: /\bcreci\b/ },
  {
    label: 'corretor',
    weight: 3,
    pattern: /\b(corretor|corretora|corretores|corretagem)\b/,
  },
  {
    label: 'imobiliaria',
    weight: 3,
    pattern: /\b(imobiliaria|real estate|re\/max|remax)\b/,
  },
  {
    label: 'parceria',
    weight: 2,
    pattern:
      /\b(parceria|parceiro|parceira|captador|captadora|comissao|carteira|portfolio)\b/,
  },
  {
    label: 'grupo de corretores',
    weight: 4,
    pattern:
      /\b(chat:|grupo)\b.{0,80}\b(corretores|corretor|imoveis|imobiliarias|permuta|repasse)\b/,
  },
];
...
function classifyLead(lead, activities, tags) {
  ...
  if (partnerScore >= 4 && sellerScore < 5 && buyerScore < 6) {
    ...
    classification =
      sellerScore >= buyerScore ? 'Vendedor Fazenda' : 'Comprador Fazenda';
    tagsToAdd.push(
      sellerScore >= buyerScore
        ? 'intent-vendedor-fazenda'
        : 'intent-comprador-fazenda'
    );
...
function toMarkdown(report) {
  ...
  const sellerRows =
    report.classified
      ...
      .map(
        (item) =>
          `| ${item.name || ''} | ${item.phone || ''} | ${item.source || ''} | ${item.scores.sellerScore} | ${item.signals.seller.join(', ')} |`
      )
      .join('\n') || '| Nenhum | - | - | - | - |';
  const buyerRows =
    report.classified
      ...
      .map(
        (item) =>
          `| ${item.name || ''} | ${item.phone || ''} | ${item.source || ''} | ${item.scores.buyerScore} | ${item.signals.buyer.join(', ')} |`
      )
      .join('\n') || '| Nenhum | - | - | - | - |';
  const partnerRows =
    report.classified
      ...
      .map(
        (item) =>
          `| ${item.name || ''} | ${item.phone || ''} | ${item.source || ''} | ${item.signals.partner.join(', ')} |`
      )
      .join('\n') || '| Nenhum | - | - | - |';
...
const counts = {
  ...
  'Vendedores de fazenda': classified.filter(
    (item) => item.intent === 'vendedor'
  ).length,
  'Compradores de fazenda': classified.filter(
    (item) => item.intent === 'comprador'
  ).length,
  'Corretores/parcerias': classified.filter(
    (item) => item.intent === 'parceria'
  ).length,
};
