...
async function clean() {
  ...
  const { error } = await supabase
    .from('properties')
    .delete()
    .neq('id', '00000000-0000-0000-0000-000000000000'); // Delete all (using neq id 0 which is UUID safe trick if column is uuid, or use gte id 0 if int. Properties usually has text/uuid id. Let's assume we can just delete all without filter or use valid filter)
  ...
  const { error: delError, count } = await supabase
    .from('properties')
    .delete()
    .gte('price', 0); // Delete all with price >= 0
  ...
  if (delError) {
    ...
    console.log(
      'Tentando estratégia alternativa (deletar por titulo != vazio)'
    );
