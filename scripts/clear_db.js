...
async function clearDatabase() {
  ...
  const { error: errLeads } = await supabase
    .from('leads')
    .delete()
  ...
  const { error: errFavs } = await supabase
    .from('favorites')
    .delete()
    .neq('id', '00000000-0000-0000-0000-000000000000');
  if (errFavs)
    console.log(
      '⚠️ Aviso ao limpar favorites (pode não existir):',
      errFavs.message
    );
  ...
  if (error) {
    ...
    console.log(
      '✅ Banco de dados limpo com sucesso! Todos os imóveis foram removidos.'
    );
  }
}
