...
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  console.error(
    'Certifique-se de que VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY estão no .env'
  );
  process.exit(1);
}
...
async function checkBuckets() {
  ...
  if (isMinioConfigured()) {
    console.log(
      '  MinIO/S3 configurado. Buckets Supabase legados nao serao validados neste diagnostico.'
    );
    return true;
  }
  ...
  try {
    const { data: buckets, error } = await (
...
async function checkPropertyImages() {
  ...
  try {
      ...
      if (images.length === 0) {
        ...
        for (let i = 0; i < images.length; i++) {
          ...
          if (!imageUrl.includes('supabase.co')) {
            console.log(
              `     ⚠️  Imagem ${i + 1}: URL externa - ${imageUrl.substring(0, 50)}...`
            );
            continue;
          }
    ...
    if (brokenImages > 0) {
      ...
      console.log(
        '   → Execute o script de migração para transferir as imagens'
      );
    }
...
async function listStorageFiles() {
  ...
  if (isMinioConfigured()) {
    console.log(
      '  MinIO/S3 configurado. Use o console MinIO ou o inventario de objetos para listar midias.'
    );
    return;
  }
