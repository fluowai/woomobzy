...
console.log(
  'For security reasons, Supabase JS client cannot execute raw SQL (DROP TABLE, etc) directly unless an RPC function is exposed.'
);
console.log(
  "Please copy the content of 'migrate_to_single_tenant.sql' and run it in your Supabase SQL Editor."
);
...
async function cleanData() {
  ...
  if (orgs && orgs.length > 1) {
    console.log(
      `Found ${orgs.length} organizations. Keeping the first one: ${orgs[0].name}`
    );
    ...
    /* 
        ...
        */
    console.log(
      'Skipping delete to avoid FK constraint violations before SQL script is run.'
    );
  } else {
