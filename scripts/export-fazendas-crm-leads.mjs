...
async function buildWorkbook(rows, rawClients, metadata) {
  ...
  const interestedCount = rows.filter((row) =>
    row.tipos.toLowerCase().includes('interessado')
  ).length;
  ...
  summary.getRange('A1:D1').values = [
    ['Exportacao de leads - Fazendas Brasil CRM'],
  ];
  ...
  leads.getRangeByIndexes(1, 0, rows.length, headers.length).values = rows.map(
    (row) => headers.map((header) => row[header] ?? '')
  );
  leads.getRangeByIndexes(
    0,
    0,
    rows.length + 1,
    headers.length
  ...
  raw.getRangeByIndexes(1, 0, rawClients.length, 2).values = rawClients.map(
    (client) => [client.id || '', JSON.stringify(client)]
  );
...
for (let page = 1; page <= totalPages; page += 1) {
  ...
  console.log(
    `page=${page}/${totalPages} contacts=${pageClients.length} total=${clientsById.size}`
  );
  if (pageClients.length === 0) break;
}

const rawClients = [...clientsById.values()].sort(
...
await fs.writeFile(
  jsonPath,
  JSON.stringify(
    { exportedAt: new Date().toISOString(), totalCount, rows: rawClients },
    null,
    2
  ),
  'utf8'
...
await buildWorkbook(rows, rawClients, {
