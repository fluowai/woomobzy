...
async function run() {
  ...
  try {
    ...
    const withExternal = props.filter(p => p.external_id && p.external_id.length > 0);
    const withoutExternal = props.filter(p => !p.external_id || p.external_id.length === 0);
    const withImages = props.filter(p => p.images && Array.isArray(p.images) && p.images.length > 0);
