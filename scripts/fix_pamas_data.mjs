...
async function fixPamasData() {
  ...
  const brokenTitles = properties?.filter(p => !p.title || p.title === '.' || p.title.trim() === '') || [];
  ...
  for (const [ext, props] of duplicates) {
    ...
    console.log(`  Keeping: ${toKeep.id} (${toKeep.title?.substring(0, 50)}, ${toKeep.images?.length || 0} images)`);
  ...
  const withImages = finalProps?.filter(p => p.images && p.images.length > 0) || [];
  const withoutImages = finalProps?.filter(p => !p.images || p.images.length === 0) || [];
