...
async function checkMinIOForOrg(orgId, orgName) {
  ...
  try {
    ...
    const stream = minioClient.listObjectsV2(
      process.env.MINIO_MEDIA_BUCKET || 'imobfluow',
      orgId + '/',
      true
    );
...
async function run() {
  ...
  console.log('\n=== COMPARISON ===');
  console.log(
    `Pamas DB: ${pamasProps?.length || 0} | Pamas Website: ${pamasUrls.length}`
  );
  console.log(
    `Mega DB: ${megaProps?.length || 0} | Mega Website: ${megaUrls.length}`
  );

  // 5. Find missing properties
  const pamasExternalIds = new Set(
    (pamasProps || []).map((p) => p.external_id).filter(Boolean)
  );
  const megaExternalIds = new Set(
    (megaProps || []).map((p) => p.external_id).filter(Boolean)
  );
  ...
  const missingFromPamas = pamasUrls.filter(
    (url) => !pamasExternalIds.has(url)
  );
