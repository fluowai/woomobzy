...
if (!supabaseUrl || !supabaseKey) {
  console.error(
    '❌ Erro: VITE_SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY (ou VITE_SUPABASE_ANON_KEY) precisam estar no .env'
  );
  process.exit(1);
}
...
const LISTING_URL_TEMPLATE =
  'https://www.fazendasbrasil.com.br/imobiliaria/imoveis/0/';
...
async function uploadImageToSupabase(imgUrl, propertyId) {
  try {
    ...
    console.warn(
      `      ⚠️ Falha ao baixar/enviar imagem ${imgUrl}: ${err.message}`
    );
    return null;
  }
}
...
async function processProperty(url) {
  ...
  try {
    ...
    function isValidImage(src) {
      ...
      if (lower.includes('/gallery/') || lower.includes('/assets/'))
        return false;
    ...
    console.log(
      `      📸 Encontradas ${originalImages.length} imagens originais (Filtradas).`
    );
    ...
    if (originalImages.length === 0) {
      console.log(
        '      ⚠️ Nenhuma imagem válida encontrada após filtro estrito.'
      );
    }
    ...
    if (upsertErr) {
      console.error(
        `      ❌ Erro ao salvar imóvel inicial: ${upsertErr.message}`
      );
      return;
    }
    ...
    if (finalImages.length > 0) {
      ...
      if (updateErr)
        console.error(
          `      ❌ Erro ao atualizar imagens finais: ${updateErr.message}`
        );
      else
        console.log(
          `      ✅ Imagens atualizadas com sucesso! (${finalImages.length})`
        );
    }
    // }
  } catch (err) {
...
async function runMigration() {
  console.log(
    '🚀 Iniciando Migração V2 - Fazendas Brasil (Com Imagens Supabase)'
  );
