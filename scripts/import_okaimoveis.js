  ...
  try {
    ...
    cards.each((i, el) => {
      ...
      card.find('.imov_caract span').each((j, span) => {
        ...
        } else if (iconClass.includes('expand') || text.includes('m²')) {
          // Sometimes area is in card
          features.area =
            parseFloat(text.replace('m²', '').replace(',', '.')) || 0;
        }
      });
...
async function processDetail(url, basicData) {
  try {
    ...
    $(
      'a[data-fancybox="gallery"], a[data-lightbox="roadtrip"], .carousel-item img, .slide img'
    ).each((i, el) => {
    ...
    if (basicData.features.area === 0) {
      ...
      $('li, p, span').each((i, el) => {
        ...
        if ((t.includes('Área') || t.includes('Area')) && t.includes('m²')) {
          ...
          if (match) {
            basicData.features.area = parseFloat(
    ...
    if (error) {
      console.error(
        `      ❌ Erro ao salvar "${basicData.title}":`,
        error.message
      );
    } else {
