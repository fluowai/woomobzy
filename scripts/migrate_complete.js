...
async function scrapePropertyDetails(url) {
  ...
  try {
    ...
    // Preço
    let price = 0;
    const priceText = $('.preco, .price, [class*="preco"]')
      .first()
      .text()
      .trim();
    ...
    // Descrição
    const description =
      $('.descricao, .description, [class*="desc"]').first().text().trim() ||
      '';
    ...
    $(
      '.galeria img, .gallery img, [class*="galeria"] img, [class*="gallery"] img'
    ).each((i, el) => {
    ...
    $('img').each((i, el) => {
      ...
      if (isValidImageUrl(src)) {
        ...
        if (
          fullUrl.includes('/exportacao/') ||
          fullUrl.includes('/fotos/') ||
          fullUrl.includes('/imovel/')
        ) {
          imageUrls.add(fullUrl);
        }
      }
...
async function migrateAllProperties() {
  ...
  if (leadsError)
    console.log('   ⚠️ Aviso ao limpar leads:', leadsError.message);
  ...
  if (favsError)
    console.log('   ⚠️ Aviso ao limpar favoritos:', favsError.message);
  ...
  for (let i = 0; i < propertyLinks.length; i++) {
    ...
    for (let j = 0; j < propertyData.images.length; j++) {
      ...
      console.log(
        `      [${j + 1}/${propertyData.images.length}] Processando...`
      );
