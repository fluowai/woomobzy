...
async function scrapePropertyDetails(url) {
    ...
    const description = $('.imovel-descricao, .descricao, #descricao').text().trim() || title;
    ...
    if (images.length === 0) {
        $('img').each((i, el) => {
            let src = $(el).attr('src') || $(el).attr('data-src');
            if (src && (src.includes('imovel') || src.includes('imoveis') || src.includes('fotos'))) {
                if (src.startsWith('/')) src = BASE_URL + src;
                images.push(src);
            }
        });
...
async function run() {
    ...
    console.log(`Found ${propertyLinks.size} unique properties. Scraping details...`);
