...
async function run() {
  ...
  const { data: orgSuperAdminData, error: orgSuperAdminErr } = await supabase
    .from('organizations')
    .upsert(
      { slug: 'super-admin-org', name: 'Super Admin Org', status: 'ACTIVE', is_reseller: true },
      { onConflict: 'slug', ignoreDuplicates: false }
    )
    .select('id')
    .single();
  ...
  // Users
  console.log('\nCreating users...');
  await upsertUser('admin-urbana-a@imobzy.test', 'imobzyOnda0!', 'Admin Urbana A', 'admin', orgUrbanaA);
  await upsertUser('corretor-urbana-b@imobzy.test', 'imobzyOnda0!', 'Corretor Urbana B', 'broker', orgUrbanaB);
  await upsertUser('admin-rural-a@imobzy.test', 'imobzyOnda0!', 'Admin Rural A', 'admin', orgRuralA);
  
  await upsertUser('superadmin@imobzy.test', 'imobzyOnda0!', 'Super Admin', 'superadmin', orgSuperAdmin);
  await upsertUser('megaadmin@imobzy.test', 'imobzyOnda0!', 'Mega Admin', 'superadmin', null);
