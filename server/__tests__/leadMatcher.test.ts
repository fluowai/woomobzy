...
describe('Lead Property Matcher - Core Logic', () => {
  it('should classify lead profile based on text', () => {
    const ruralLead = {
      ...
      notes: 'Quero pecuaria e agricultura',
    };
    const urbanLead = { name: 'Joao', notes: 'Busco apartamento 3 quartos' };
    const mixedLead = { name: 'Maria', notes: 'Quero chacara ou apartamento' };

    const ruralText = [ruralLead.name, ruralLead.notes]
      .filter(Boolean)
      .join(' ');
    const urbanText = [urbanLead.name, urbanLead.notes]
      .filter(Boolean)
      .join(' ');
    const mixedText = [mixedLead.name, mixedLead.notes]
      .filter(Boolean)
      .join(' ');
    ...
    const ruralHits = RURAL_KEYWORDS.filter((k) =>
      ruralText.toLowerCase().includes(k)
    ).length;
    const urbanHitsRural = URBAN_KEYWORDS.filter((k) =>
      ruralText.toLowerCase().includes(k)
    ).length;
    expect(ruralHits).toBeGreaterThan(0);
    expect(urbanHitsRural).toBe(0);

    const urbanHits = URBAN_KEYWORDS.filter((k) =>
      urbanText.toLowerCase().includes(k)
    ).length;
    expect(urbanHits).toBeGreaterThan(0);

    const ruralHitsMixed = RURAL_KEYWORDS.filter((k) =>
      mixedText.toLowerCase().includes(k)
    ).length;
    const urbanHitsMixed = URBAN_KEYWORDS.filter((k) =>
      mixedText.toLowerCase().includes(k)
    ).length;
  ...
  it('should detect budget range in text', () => {
    ...
    const upToMatch = textUpTo.match(
