...
/**
 * GET /api/cobranca
 * Lista cobranças/boletos do tenant
 */
 ...
 * Gera cobranças mensais para todos os contratos ativos do tenant
 */
router.post('/gerar-mensal', verifyAuth, requireTenant, async (req, res) => {
  try {
 ...
 */
router.put('/:id/pagar', verifyAuth, requireTenant, async (req, res) => {
  try {
    const { id } = req.params;
 ...
 * Cancela boleto
 */
router.put('/:id/cancelar', verifyAuth, requireTenant, async (req, res) => {
  try {
    const { id } = req.params;
 ...
 * Dashboard financeiro completo do tenant
 */
router.get(
