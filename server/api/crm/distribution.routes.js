...
import { distributeLead, bulkDistributeLeads, DISTRIBUTION_STRATEGIES } from '../../services/leadDistributionService.js';
...
router.post('/distribute', verifyAuth, requireTenant, async (req, res) => {
  try {
    ...
    const validStrategy = Object.values(DISTRIBUTION_STRATEGIES).includes(strategy)
      ? strategy
    ...
    if (!broker) {
      return res.status(404).json({ error: 'Nenhum corretor disponível para distribuição' });
    }
...
router.post('/bulk-distribute', verifyAuth, requireTenant, async (req, res) => {
  try {
    ...
    if (!Array.isArray(lead_ids) || lead_ids.length === 0) {
      return res.status(400).json({ error: 'lead_ids deve ser um array não vazio' });
    }
    ...
    const results = await bulkDistributeLeads(req.orgId, lead_ids, validStrategy);
 ...
 */
router.get('/distribution-strategies', verifyAuth, requireTenant, async (req, res) => {
  res.json({
    success: true,
    strategies: Object.entries(DISTRIBUTION_STRATEGIES).map(([key, value]) => ({
      key,
      value,
      label: {
