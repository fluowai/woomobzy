...
router.get('/leads', verifyAuth, requireTenant, async (req, res) => {
  try {
    ...
    if (intent === 'comprador')
      query = query.eq('classification', 'Comprador Fazenda');
    if (intent === 'vendedor')
      query = query.eq('classification', 'Vendedor Fazenda');
    if (intent === 'parceria')
      query = query.eq('classification', 'Corretor/Parceria');
    ...
    const leads = (hasMore ? data.slice(0, limit) : data || []).map(
      serializeKanbanLead
    );
  ...
  try {
    ...
    const [
      ...
      supabase
        ...
        .select(
          'id, lead_id, type, description, metadata, created_at, profiles(name)'
        )
...
router.post('/leads', verifyAuth, requireTenant, async (req, res) => {
  try {
    ...
    if (!name || !phone) {
      return res
        .status(400)
        .json({ error: 'Nome e telefone sao obrigatorios' });
    }

    if (!isValidBRPhone(normalizedPhone)) {
      return res
        .status(400)
        .json({ error: 'Telefone brasileiro valido e obrigatorio' });
    }
    ...
    if (existingLead) {
      const { data: updated, error } = await supabase
        .from('leads')
        .update({
          name: isPlaceholderLeadName(existingLead.name)
            ? name
            : existingLead.name,
          ...
          organic_channel:
            req.body.organic_channel || existingLead.organic_channel,
          campaign: req.body.campaign || existingLead.campaign,
          notes: [existingLead.notes, req.body.notes]
            .filter(Boolean)
            .join('\n\n'),
          budget: req.body.budget || existingLead.budget,
          aptitude_interest:
            req.body.aptitude_interest || existingLead.aptitude_interest,
    ...
    res
      ...
      .json({ success: true, lead: matchedLead });
  } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      if (!type || !description) {
        return res
          .status(400)
          .json({ error: 'Tipo e descricao sao obrigatorios' });
      }
      ...
      res
        .status(201)
        .json({ success: true, activity: data, lead: updatedLead });
    } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: `${ids.length} leads excluidos com sucesso`,
      });
    } catch (err) {
