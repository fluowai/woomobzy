...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowed.includes(file.mimetype)) {
      ...
      cb(
        new Error('Tipo de arquivo nao permitido. Use PDF, JPEG, PNG ou DOC.')
      );
    }
  },
});
...
export function isDocumentWorkerWebhookAuthorized(req) {
  const documentWebhookSecret = String(
...
router.get('/:propertyId', verifyAuth, requireTenant, async (req, res) => {
  try {
    ...
    const documents = await DocumentService.listByProperty(
      propertyId,
      req.orgId
    );
...
router.post('/webhook/worker-result', async (req, res) => {
  ...
  try {
    ...
    if (!result || typeof result !== 'object') {
      return res
        .status(400)
        .json({ error: 'document_id e result obrigatorios' });
    }
