...
router.get('/lead-ads/config', verifyAuth, requireTenant, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'id, meta_form_id, meta_campaign_id, meta_ad_id, assigned_agent_id, priority, active, created_at, updated_at'
      )
      .eq('organization_id', req.orgId)
    ...
    try {
      ...
      if (!formId && !campaignId && !adId) {
        return res.status(400).json({
          error:
            'Informe ao menos um identificador: form_id, campaign_id ou ad_id',
        });
      }
      ...
      if (targetCount > 1) {
        return res.status(400).json({
          error:
            'Informe apenas um identificador por vez: form_id, campaign_id ou ad_id',
        });
      }
      ...
      const { data: existing } = await supabase
        ...
        .eq('organization_id', req.orgId)
        .or(
          `meta_form_id.${metaFormId || 'null'},meta_campaign_id.${metaCampaignId || 'null'},meta_ad_id.${metaAdId || 'null'}`
        )
        .maybeSingle();

      if (existing?.id) {
        return res.status(409).json({
          error: 'Já existe uma regra para este identificador Meta',
  ...
  async (req, res) => {
    try {
      ...
      let query = supabase
        ...
        .select(
          'id, meta_lead_id, event_type, status, error_message, processed_at, payload'
        )
        .eq('organization_id', req.orgId)
