  ...
  try {
    ...
    if (!lat && !lon && !code && !name) {
      return res.status(400).json({ 
        success: false, 
        error: 'Forneca ao menos um criterio de busca (coordenadas, codigo ou nome).' 
      });
    }
...
router.post('/find-car-by-location', verifyAuth, requireTenant, async (req, res) => {
  ...
  try {
    if (googleMapsUrl && (!lat || !lng)) {
      ...
      if (!coords) {
        return res.status(400).json({ success: false, error: 'Nao foi possivel extrair coordenadas do link informado.' });
      }
      lat = coords.lat;
      lng = coords.lng;
    }
    ...
    if (!lat || !lng) {
      return res.status(400).json({ success: false, error: 'Latitude e Longitude sao obrigatorios.' });
    }
    ...
    const ufsToTry = locationInfo.uf ? [locationInfo.uf] : ['MT', 'GO', 'MS', 'PA', 'TO', 'BA', 'MG'];
    let matches = [];
    let matchMode = 'none';
    let queriedUf = null;
    ...
    res.status(500).json({ success: false, error: 'Erro interno ao processar busca de localizacao.', message: error.message });
  }
});
