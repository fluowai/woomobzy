...
if (!supabaseServiceKey || !jwtSecret) {
  console.error(
    '❌ ERRO CRÍTICO: SUPABASE_SERVICE_ROLE_KEY ou SUPABASE_JWT_SECRET faltando!'
  );
}
  ...
  try {
    ...
    if (actorError || actorProfile?.role !== 'superadmin') {
      await logAudit(
        ...
        { reason: 'Unauthorized', target: targetUserId },
        req.ip
      );
      return res
        .status(403)
        .json({ error: 'Acesso negado. Apenas Super Admins.' });
    }
    ...
    await logAudit(
      actorId,
      'IMPERSONATE_Start',
      { sessionId: session.id, target: targetUserId, tenant: tenantId },
      req.ip
    );
