...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST' && action === 'bulk') {
    try {
      ...
      if (!Array.isArray(updates) || updates.length === 0) {
        return res
          .status(400)
          .json({ error: 'Updates deve ser um array não vazio' });
      }
      ...
      return res
        .status(500)
        .json({ error: 'Erro ao processar atualização em massa' });
    }
  }
  ...
  if (req.method === 'POST' && action === 'seed') {
    try {
      ...
      if (count > 0)
        return res.status(400).json({ error: 'Textos já existem no banco.' });
      return res.json({
        success: true,
        message: 'Execute o arquivo seed_site_texts.sql no Supabase',
      });
    } catch (error) {
    ...
    if (req.method === 'GET') {
      ...
      if (error)
        return res
          ...
          .json({ error: 'Erro ao buscar texto' });
