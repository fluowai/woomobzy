...
export class WahaClient {
  ...
  async health() {
    const result = await this.request('/api/server/status', { method: 'GET' }).catch(() => null);
    if (result?.ok) return result;
    const fallback = await this.request('/api/health', { method: 'GET' }).catch(() => null);
  ...
  async ensureSession(instance) {
    ...
    const result = await this.request('/api/sessions', { method: 'POST', body });
    if (!result.ok && result.status === 409) return { name };
    if (!result.ok) throw new Error(`WAHA session create failed: ${result.status} ${result.data?.error || ''}`);
    return { name };
  }

  async getSessionByName(name) {
    const result = await this.request(`/api/sessions/${encodeURIComponent(name)}`, { method: 'GET' }).catch(() => null);
  ...
  async requestPairingCode(instance, phone) {
    ...
    if (!result?.ok) throw new Error(`WAHA pairing code failed: ${result?.status || 503}`);
    return result.data;
  }

  async logout(instance) {
    const name = this.sessionName(instance);
    await this.request(`/api/sessions/${encodeURIComponent(name)}/logout`, { method: 'POST' }).catch(() => null);
  }

  async deleteSession(instance) {
    const name = this.sessionName(instance);
    await this.request(`/api/sessions/${encodeURIComponent(name)}`, { method: 'DELETE' }).catch(() => null);
  }
  ...
  async sendText(instance, chatJid, text) {
    ...
    const result = await this.request('/api/sendText', { method: 'POST', body, timeoutMs: 10000 }).catch(() => null);
    if (!result?.ok) throw new Error(`WAHA sendText failed: ${result?.status || 503} ${result?.data?.error || ''}`);
  ...
  async sendMedia(instance, chatJid, media) {
    ...
    const result = await this.request(endpoint, { method: 'POST', body, timeoutMs: 15000 }).catch(() => null);
    if (!result?.ok) throw new Error(`WAHA ${endpoint} failed: ${result?.status || 503}`);
    return result.data || { id: randomUUID() };
  }
  ...
  async sendSeen(instance, chatJid) {
    ...
    await this.request('/api/sendSeen', { method: 'POST', body }).catch(() => null);
  }

  async getChats(instance) {
    const name = this.sessionName(instance);
    const result = await this.request(`/api/chats?session=${encodeURIComponent(name)}`, { method: 'GET' }).catch(() => null);
    if (!result?.ok) return [];
  ...
  buildSessionConfig(instance) {
    ...
    if (this.webhookUrl) {
      const webhookTarget = new URL(`${this.webhookUrl}/api/whatsapp/internal/waha/webhook`);
  ...
  mediaEndpoint(type) {
    switch (type) {
      ...
      case 'audio': return '/api/sendVoice';
