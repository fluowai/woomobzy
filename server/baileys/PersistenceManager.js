...
export class PersistenceManager {
  ...
  async updateStatus(instanceId, status, extras = {}) {
    try {
      const update = { status, updated_at: new Date().toISOString(), ...extras };
      if (status === 'connected') update.qr_code = null;
      const { error } = await getSupabase()
        .from('whatsapp_instances')
        .update(update)
        .eq('id', instanceId);
      if (error) console.error(`[PersistenceManager] ❌ Erro ao atualizar status:`, error.message);
    } catch (e) {
  ...
  async saveQRCode(instanceId, qrCode) {
    try {
      ...
      console.error(`[PersistenceManager] ❌ Erro ao salvar QR:`, e.message);
    }
  }
