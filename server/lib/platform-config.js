...
export const PLATFORM_PRODUCT_NAME = process.env.PLATFORM_PRODUCT_NAME || 'Imob';
export const PLATFORM_COMMERCIAL_NAME =
  process.env.PLATFORM_COMMERCIAL_NAME || `${PLATFORM_BRAND_NAME} ${PLATFORM_PRODUCT_NAME}`;
export const PANEL_URL = (process.env.APP_URL || process.env.VITE_PANEL_URL || 'https://imob.wootech.com.br').replace(/\/$/, '');
export const PUBLIC_APP_URL = (process.env.PUBLIC_APP_URL || process.env.VITE_PUBLIC_APP_URL || PANEL_URL).replace(/\/$/, '');
...
export function getPlatformOriginList() {
  return Array.from(
    new Set(
      [PANEL_URL, PUBLIC_APP_URL, ...LEGACY_PANEL_HOSTS.map((host) => `https://${host}`), ...LEGACY_PUBLIC_HOSTS.map((host) => `https://${host}`)]
        .filter(Boolean)
