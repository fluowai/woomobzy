...
const allTools = [
  ...
  },
  {
    ...
    description: 'Simula um financiamento imobiliario usando a Tabela Price ou juros simples, devolvendo o valor aproximado da parcela.',
    parameters: {
      type: 'object',
      properties: {
        valor_imovel: {
  ...
  },
  {
    ...
    description: 'Atualiza o status/etapa do lead no Kanban (CRM) baseado na acao do cliente (ex: pediu simulacao, agendou visita).',
    parameters: {
      type: 'object',
      properties: {
        nova_etapa: {
          type: 'string',
          description: 'A nova etapa no Kanban. Use apenas: "Novo", "Em andamento", "Contato", "Agendado", "Proposta", "Fechado" ou "Perdido".',
        },
      },
      required: ['nova_etapa'],
    },
  },
  {
    ...
    description: 'Avalia o perfil do cliente e atualiza o CRM com a temperatura (frio, morno, quente) e um motivo baseado na interacao.',
    parameters: {
      type: 'object',
      properties: {
        temperatura: {
          type: 'string',
          description: 'A temperatura do lead. Opcoes permitidas: "frio", "morno" ou "quente".',
        },
        motivo: {
          type: 'string',
          description: 'Breve justificativa do por que o lead recebeu essa temperatura (ex: "Demonstrou muito interesse em fechar na hora").',
        },
      },
  ...
  },
  {
    ...
    description: 'Gera um audio a partir de texto (Text-to-Speech) e envia para o cliente pelo WhatsApp. Use isto quando o cliente pedir para enviar um audio ou quando a resposta for longa e acolhedora.',
    parameters: {
      type: 'object',
      properties: {
        texto_falado: {
          type: 'string',
          description: 'O texto exato que deve ser transformado em audio e enviado ao cliente.',
        },
      },
...
export class AgentOrchestrator {
  ...
  async _ensureModel(agentToolsConfig) {
    ...
    if (agentToolsConfig?.includes('simulador-financiamento')) {
      activeFunctionDeclarations.push(allTools.find(t => t.name === 'simular_financiamento'));
    }
    if (agentToolsConfig?.includes('mover-etapa-funil') || agentToolsConfig?.includes('crm')) {
      activeFunctionDeclarations.push(allTools.find(t => t.name === 'atualizar_etapa_crm'));
    }
    if (agentToolsConfig?.includes('neural-sales')) {
      activeFunctionDeclarations.push(allTools.find(t => t.name === 'qualificar_lead'));
    }
    if (agentToolsConfig?.includes('voice-ai')) {
      activeFunctionDeclarations.push(allTools.find(t => t.name === 'enviar_audio_whatsapp'));
    }
  ...
  async executeToolCall(functionCall, organizationId, leadId) {
    ...
    try {
      ...
      if (name === 'simular_financiamento') {
        ...
        if (principal <= 0) return { erro: 'Valor de entrada maior ou igual ao imovel. Nao ha necessidade de financiamento.' };
        
        // Simples: juros de 9.5% ao ano (0.0079 a.m.) Tabela Price
        const i = 0.0079;
        const n = args.prazo_meses;
        const parcela = principal * (i * Math.pow(1 + i, n)) / (Math.pow(1 + i, n) - 1);
      ...
      if (name === 'atualizar_etapa_crm') {
        if (!leadId) return { erro: 'Lead nao identificado. Nao e possivel mover funil sem um lead salvo.' };
        
        const validStages = ['Novo', 'Em andamento', 'Contato', 'Agendado', 'Proposta', 'Fechado', 'Perdido'];
        if (!validStages.includes(args.nova_etapa)) {
          return { erro: `Etapa invalida. Use uma das seguintes: ${validStages.join(', ')}` };
        }
        ...
        return { sucesso: true, mensagem: `Lead movido com sucesso para a etapa: ${args.nova_etapa}.` };
      }

      if (name === 'qualificar_lead') {
        if (!leadId) return { erro: 'Lead nao identificado. Impossivel salvar qualificacao.' };
        
        const { data: leadData } = await supabase.from('leads').select('ai_profile').eq('id', leadId).single();
        ...
        if (error) return { erro: 'Erro ao atualizar perfil do lead: ' + error.message };
        return { sucesso: true, mensagem: `Lead qualificado como ${args.temperatura}. Motivo salvo.` };
      }

      if (name === 'enviar_audio_whatsapp') {
        return { 
          sucesso: true, 
          instrucao_interna: 'A infraestrutura processara isso em breve. Na sua resposta ao cliente, inclua a tag [VOICE_AI] seguida do texto que deve ser falado. Exemplo: [VOICE_AI]Ola, como posso ajudar?[/VOICE_AI]' 
        };
      }
