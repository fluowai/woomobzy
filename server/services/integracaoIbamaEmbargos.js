...
export class IntegracaoIbamaEmbargos {
        ...
        try {
          ...
          return {
            ...
            embargos: (response.data || []).map((e) => ({
              ...
              area_embargada_ha: parseFloat(
                e.areaEmbargada || e.area_embargada || 0
              ),
    ...
    return this._fetchWithCache(
      ...
      async () => {
        try {
          ...
          return {
            total_embargos: null,
            erro: 'API IBAMA municipio indisponivel',
            fonte: 'IBAMA',
          };
        }
      },
  ...
  static async enrichPropertyWithEmbargos(features, lat, lng, municipio, uf) {
    const [porCoordenada, porMunicipio] = await Promise.all([
      ...
      municipio && uf
        ? this.consultarEmbargosPorMunicipio(municipio, uf)
        : Promise.resolve(null),
    ]);
    return {
      embargos_proximos: porCoordenada,
      embargos_municipio: porMunicipio,
    };
  }
  ...
  static _normalizarEmbargos(data) {
    const lista = Array.isArray(data)
      ? data
      : data?.embargos || data?.features || [];
    return {
      total_embargos: lista.length,
      area_total_embargada_ha: lista.reduce((s, e) => {
        return (
          s + parseFloat(e.areaEmbargada || e.area_embargada || e.area_km2 || 0)
        );
      }, 0),
      embargos: lista.map((e) => ({
        auto_infracao:
          e.autoInfracao || e.numero_auto || e.properties?.auto_infracao,
        data_embargo:
          e.dataEmbargo || e.data_embargo || e.properties?.data_embargo,
        tipo: e.tipo || e.tipoInfracao || e.properties?.tipo,
        area_embargada_ha: parseFloat(
          e.areaEmbargada || e.area_embargada || e.properties?.area_km2 || 0
        ),
  ...
  static async _fetchWithCache(cacheKey, fetcher, ttlSeconds) {
    ...
    try {
      ...
      console.error(
        `[IbamaEmbargos] Erro ao buscar ${cacheKey}:`,
        error.message
      );
      return null;
    }
