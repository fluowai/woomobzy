...
export class IntegracaoMapBiomas {
        ...
        try {
          ...
          return {
            ...
            alertas: alerts.map((a) => ({
              ...
              area_ha: a.areaHa,
              classificacao: Array.isArray(a.sources)
                ? a.sources.join(', ')
                : null,
              status: a.statusName,
            })),
            area_total_alertas_ha: alerts.reduce(
              (s, a) => s + (a.areaHa || 0),
              0
            ),
            fonte: 'MapBiomas Alerta',
            token_configurado: Boolean(
              process.env.MAPBIOMAS_API_TOKEN || process.env.MAPBIOMAS_EMAIL
            ),
            api_autenticada: Boolean(
              mapbiomasSessionToken || process.env.MAPBIOMAS_API_TOKEN
            ),
            bounding_box_delta: delta,
          };
        } catch (error) {
          console.warn(
            `[MapBiomas] API erro, usando fallback: ${error.message}`
          );
        ...
        try {
          ...
          for (const item of Array.isArray(dados) ? dados : [dados]) {
            ...
            usoPorAno[ano] = {
              ...
              agropecuaria_ha: parseFloat(
                item.agropecuaria || item.agriculture || 0
              ),
          ...
          return {
            ...
            ultimo_ano_disponivel: Math.max(
              ...Object.keys(usoPorAno).map(Number),
              0
            ),
          ...
          return {
            erro: 'Dados municipais MapBiomas indisponiveis',
            fonte: 'MapBiomas',
          };
        }
      },
  ...
  static _fallbackUsoSolo(lat, lng) {
    return {
      ...
      mensagem:
        'Consulta MapBiomas indisponivel no momento. Configure MAPBIOMAS_EMAIL/MAPBIOMAS_PASSWORD ou MAPBIOMAS_API_TOKEN e tente novamente.',
    };
  }
  ...
  static async enrichPropertyWithMapBiomas(features, lat, lng, codigoIbge) {
    const [usoLocal, usoMunicipio] = await Promise.all([
      ...
      codigoIbge
        ? this.consultarUsoSoloPorMunicipio(codigoIbge)
