...
export class IntegracaoTerraBrasilis {
    ...
    return this._fetchWithCache(
      ...
      async () => {
        ...
        try {
          ...
          return {
            possui_desmatamento: null,
            erro: 'PRODES indisponivel',
            fonte: 'PRODES/INPE',
          };
        }
      },
      ...
      async () => {
        try {
          const response = await axios.get(
            `${PRODES_API}/municipios/${codigoIbge}/desmatamento`,
            {
              params: { ano: ano || '' },
              timeout: 15000,
            }
          ...
          return {
            taxa_desmatamento_km2: null,
            erro: 'API PRODES municipio indisponivel',
          };
        }
      },
      86400
  ...
  static async _fetchWithCache(cacheKey, fetcher, ttlSeconds) {
    ...
    try {
      ...
      console.error(
        `[TerraBrasilis] Erro ao buscar ${cacheKey}:`,
        error.message
      );
      return null;
    }
