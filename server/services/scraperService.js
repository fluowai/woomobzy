...
export async function runScraperScrapeOnly(targetUrl, organizationId) {
  try {
    ...
    for (let i = 0; i < linksToProcess.length; i++) {
      try {
        const prop = await processPropertyScrapeOnly(
          linksToProcess[i],
          organizationId
        );
    ...
    throw new Error('Erro ao acessar página e tentar extração.');
  }
}
...
export async function processPropertyScrapeOnly(url, organizationId) {
  try {
    ...
    const title =
      $('h1').text().trim() || $('h2').first().text().trim() || 'Sem Título';
    ...
    const images = await migratePropertyImages(
      imageCandidates,
      organizationId,
      {
        limit: 10,
