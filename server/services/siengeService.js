...
export class SiengeService {
  ...
  async testConnection() {
    console.log(`[Sienge] Testing connection for organization ${this.organizationId}...`);
  ...
  async syncCustomers(customers) {
    ...
    // Mock API call
    return customers.map(c => ({ ...c, sienge_id: `SIENGE-${Date.now()}-${Math.floor(Math.random() * 1000)}` }));
  }

  async getFinancialTransfers(projectId) {
    console.log(`[Sienge] Fetching financial transfers for project ${projectId}...`);
    // Mock API call
    return [
      { clientName: 'John Doe', amount: 250000.00, status: 'analysis', expected_date: '2026-08-15' }
    ];
  }
}
