...
async function getApiKeys(organizationId) {
  ...
  try {
    let query = supabase.from('site_settings').select('integrations');
...
export const extractProperties = async (url, organizationId) => {
  try {
    ...
    const processedProperties = await mapWithConcurrency(
      limitedProperties,
      3,
      async (prop) => {
        ...
        const detailImages = sourceUrl
          ? await extractDetailPageImages(sourceUrl)
          : [];
        const aiImages = sanitizePropertyImageUrls(
          prop.images || [],
          sourceUrl || url,
          15
        );
        const fallbackImages =
          limitedProperties.length === 1
            ? listingImageCandidates.slice(0, 15)
            : [];
...
async function extractDetailPageImages(sourceUrl) {
  try {
    ...
    console.warn(
      `[Import Images] Falha ao abrir pagina do imovel ${sourceUrl}:`,
      error.message
    );
    return [];
  }
...
function absolutizeHtmlUrls($, baseUrl) {
  $(
    '[href], [src], [data-src], [data-original], [data-foto], [srcset], [data-srcset]'
  ).each((_, el) => {
    const node = $(el);
    ...
    for (const attr of ['srcset', 'data-srcset']) {
      ...
      const normalized = value
        ...
        .map((item) => {
          ...
          return absolute
            ? `${absolute}${descriptor ? ` ${descriptor}` : ''}`
            : '';
        })
        .filter(Boolean)
        .join(', ');
...
export const cloneSite = async (url, organizationId) => {
  ...
  try {
    ...
    throw new Error(
      'No valid AI API keys found. Please configure OpenAI, Gemini or Groq in Settings.'
    );
  } catch (error) {
