...
export const matchLeadWithProperties = async (
  ...
  const response = await callSecureAI(
    prompt,
    'Você é um consultor imobiliário especialista em inteligência de mercado e matching de leads.',
    {
      jsonMode: true,
...
export const generateCollectionMessage = async (
  ...
  return callSecureAI(
    prompt,
    'Você é um assistente de cobrança especializado em imobiliárias e loteadoras 360.'
  );
};
...
export const geminiService = {
  ...
  extractColorsFromLogo: async (base64Image: string, mimeType: string) => {
    try {
      ...
      const response = await callSecureAI(
        prompt,
        'Você é um designer especialista em branding imobiliário.',
        { jsonMode: true }
      );
      const cleanJson = response
        .replace(/```json/g, '')
