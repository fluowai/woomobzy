...
export const LeaseDashboard: React.FC = () => {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `R$ ${(resumo?.receita_mensal || 0).toLocaleString('pt-BR')}`,
    ...
    {
      icon: TrendingUp,
      label: 'Vencer em 30 dias',
      value: String(resumo?.vencendo_30_dias || 0),
    ...
    },
    {
      icon: FileText,
      label: 'Em Andamento',
      value: String(resumo?.em_andamento || 0),
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {stats.map((stat) => (
          ...
          >
            <div
              className={`p-2.5 rounded-xl ${stat.bg} ${stat.color} w-fit mb-3`}
            >
              <stat.icon size={20} />
            </div>
            ...
            <p className="text-xl font-bold text-slate-900 italic tracking-tighter">
              {stat.value}
            </p>
          </div>
        ))}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-400">
                    ...
                    {lease.property_title || 'Sem imóvel'} · R${' '}
                    {(lease.monthly_rent || 0).toLocaleString('pt-BR')}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <span
                    className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider ${
                      PAYMENT_STATUS_LABELS[lease.payment_status || 'em_dia']
                        ? lease.payment_status === 'inadimplente'
                          ? 'bg-red-50 text-red-700'
                          : lease.payment_status === 'atrasado'
                  ...
                  <span
                    className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider ${LEASE_STATUS_COLORS[lease.status] || 'bg-slate-50 text-slate-500'}`}
                  >
