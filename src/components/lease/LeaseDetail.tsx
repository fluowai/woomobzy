...
import type {
  ...
  Signature,
  Inspection,
  LeaseHistory as LeaseHistoryType,
} from '../../types/lease';
import {
  LEASE_STATUS_LABELS,
  LEASE_STATUS_COLORS,
...
export const LeaseDetail: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try {
      const [leaseRes, invoicesRes, signaturesRes, inspectionsRes] =
        await Promise.all([
          getLease(id),
          listInvoices(id),
          listSignatures(id),
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-black/60 font-medium mt-1">
              {lease.tenant_name} ·{' '}
              {lease.property_title || 'Imóvel não vinculado'} · R${' '}
              {(lease.monthly_rent || 0).toLocaleString('pt-BR')}/mês
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <p className="text-sm font-bold text-slate-800">
            {lease.property_title || 'Não vinculado'}
          </p>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <p className="text-xs text-slate-400">
            {lease.contract_duration_months} meses
          </p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-sm font-bold text-slate-800">
            {lease.guarantee_type || 'Não definida'}
          </p>
              ...
              {[
                {
                  ...
                  type: 'Contrato',
                  date: lease.signed_at || lease.created_at,
                  icon: FileText,
                },
                {
                  name: 'Termo de Vistoria.pdf',
                  type: 'Vistoria',
                  date: lease.activated_at,
                  icon: FileText,
                },
                {
            ...
            <div className="space-y-4">
              <div className="relative pl-8 border-l-2 border-slate-100 space-y-6">
                <div className="relative">
                  ...
                  <p className="text-[10px] text-slate-400">
                    {new Date(lease.created_at).toLocaleString('pt-BR')}
                  </p>
                </div>
                {lease.activated_at && (
                  <div className="relative">
                    ...
                    <p className="text-[10px] text-slate-400">
                      {new Date(lease.activated_at).toLocaleString('pt-BR')}
                    </p>
                  </div>
                )}
                {lease.signed_at && (
                  <div className="relative">
                    ...
                    <p className="text-[10px] text-slate-400">
                      {new Date(lease.signed_at).toLocaleString('pt-BR')}
                    </p>
                  </div>
                )}
              </div>
