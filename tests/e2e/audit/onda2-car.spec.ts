...
test.describe('Onda 2 - Módulo Rural: Inteligência de Território (CAR)', () => {
  ...
  test('Deve carregar o módulo de Localização de CAR e exibir os mapas', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Busca Avançada de CAR/i }).or(page.getByRole('heading', { name: /Localizar CAR/i }).or(page.locator('text=CAR'))).first()).toBeVisible();
    ...
    await expect(page.locator('text=Cadastro Técnico').or(page.locator('text=CAR'))).toBeVisible();
