...
test.describe('Onda 3 - Super Admin: Gestão', () => {
  test('Deve acessar as principais áreas do painel Super Admin', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Imobiliárias/i }).first()).toBeVisible({ timeout: 15000 });
    
    // Navegar para Planos e Faturamento
    await page.goto(absoluteUrl('/superadmin/plans'));
    await expect(page.getByRole('heading', { name: /Planos/i }).first()).toBeVisible();
