...
const autonomyLevels = [
  ...
  },
  {
    ...
    label: 'Autônomo',
    description:
      'Responde, movimenta Kanban, agenda follow-ups e aciona humanos quando necessário.',
    icon: Zap,
  },
];
...
const toolOptions = [
  ...
  {
    id: 'notificar-corretor',
    label: 'Notificação ao corretor',
    icon: BellRing,
  },
...
const presets: TemplatePreset[] = [
  {
    ...
    description:
      'Atende leads, identifica perfil de compra e cria oportunidades qualificadas.',
  ...
  },
  {
    ...
    description:
      'Confere documentos, aponta pendências e sinaliza riscos para o time.',
    ...
    payload: {
      ...
      role: 'Analista documental',
      personality:
        'Precisa, calma e cuidadosa. Explica pendências com linguagem simples e segura.',
      instructions:
  ...
  },
  {
    ...
    description:
      'Retoma conversas, agenda retornos e reduz perda de oportunidades.',
    ...
    payload: {
      ...
      personality:
        'Persistente sem ser invasivo. Direto, cordial e sempre orientado ao próximo passo.',
      instructions:
  ...
  },
  {
    ...
    description:
      'Cruza orçamento, cidade e preferências com imóveis disponíveis.',
    ...
    payload: {
      ...
      role: 'Recomendação de imóveis',
      personality:
        'Analítica e elegante. Recomenda com base em critérios claros e contexto comercial.',
      instructions:
  ...
  },
  {
    ...
    description:
      'Organiza agenda, confirma disponibilidade e prepara o corretor.',
    tags: ['Agenda', 'Visitas'],
    ...
    payload: {
      ...
      role: 'Agendamento de visitas',
      personality:
        'Organizado, claro e prático. Confirma dados essenciais antes de acionar o corretor.',
      instructions:
  ...
  },
  {
    ...
    description:
      'Conduz propostas, negocia valores e fecha contratos com segurança.',
    ...
    payload: {
      ...
      personality:
        'Segura, persuasiva e profissional. Conduz negociações com elegância e senso de urgência.',
      instructions:
  ...
  },
  {
    ...
    description:
      'Especialista em aluguel, locação temporária e contratos de locação.',
    ...
    payload: {
      ...
      role: 'Locação de imóveis',
      personality:
        'Paciente, explicativo e focado em encontrar o imóvel ideal para moradia.',
      instructions:
  ...
  },
  {
    ...
    description:
      'Acompanha clientes após fechamento, mede satisfação e gera novas indicações.',
    ...
    payload: {
      ...
      role: 'Pós-venda e satisfação',
      personality:
        'Acolhedora, grata e atenta. Mantém relacionamento duradouro com clientes.',
      instructions:
  ...
  },
  {
    ...
    description:
      'Confere documentos jurídicos, contratos e matriculas com precisão.',
    ...
    payload: {
      ...
      personality:
        'Analítico, preciso e criterioso. Verifica cada detalhe documental.',
      instructions:
...
function buildDraftAgentReply(draft: BuilderDraft, message: string) {
  ...
  const intro =
    style === 'curto' ? 'Perfeito.' : `Perfeito, aqui e ${agentName}.`;
...
const AIAgents: React.FC = () => {
  ...
  const selectedWhatsAppInstance = useMemo(
    () =>
      whatsAppInstances.find(
        (instance) => instance.id === draft.instances?.[0]
      ),
  ...
  const lastLeadMessage =
    [...chatMessages].reverse().find((message) => message.side === 'lead')
      ?.content ||
    chatMessages[chatMessages.length - 1]?.content ||
    '';
  const previewDiagnostics = useMemo(
    () => buildPreviewDiagnostics(lastLeadMessage),
    [lastLeadMessage]
  );
  ...
  const handleQualify = async () => {
    ...
    try {
      ...
      toast.success(
        'Qualificação registrada! Isso ajuda o cerebro neural do agente.'
      );
      setRatingInput(0);
      setFeedbackInput('');
  ...
  const saveAgent = async (statusOverride?: string) => {
    ...
    const payload: BuilderDraft = {
      ...
      operation_mode:
        autonomyLevels.find(
          (level) => level.id === Number(draft.autonomy_level || 2)
        )?.label || draft.operation_mode,
      handoff: {
  ...
  const removeAgent = async () => {
    if (!selectedAgent || !confirm(`Excluir o agente "${selectedAgent.name}"?`))
      return;
    ...
    try {
      ...
      setChatMessages((current) => [
        ...current,
        {
          ...
          content:
            reply ||
            'Nao consegui responder agora. Ajuste a mensagem e tente novamente.',
        },
      ]);
      ...
      setChatMessages((current) => [
        ...current,
        {
          ...
          content:
            'Nao consegui conectar a IA agora. Ajuste a mensagem ou tente novamente em instantes.',
        },
      ]);
    } finally {
            ...
            <div className="p-5 border-b border-slate-100">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-2 text-xl font-bold tracking-tight mb-0">
                    Modelos e operação
                  </h2>
                </div>
                    ...
                    {agents.slice(0, 5).map((agent) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span
                            className={`h-2 w-2 rounded-full ${agent.is_active ? 'bg-emerald-400' : 'bg-slate-500'}`}
                          />
                        </div>
...
const Avatar: React.FC<{ label: string; gradient: string }> = ({
  label,
  gradient,
}) => (
  <div
    className={`h-11 w-11 shrink-0 rounded-lg bg-gradient-to-br ${gradient} p-[2px] shadow-sm`}
  >
...
const SectionHeading: React.FC<{
  ...
  <div>
    ...
    <h2 className="mt-1 text-lg font-bold tracking-tight text-slate-950 mb-0">
      {title}
    </h2>
    <p className="mt-1 text-sm font-medium text-slate-500 mb-0">
      {description}
    </p>
  </div>
);
...
const StatusPill: React.FC<{ status: string; compact?: boolean }> = ({
  status,
  compact,
}) => {
  ...
  return (
    <span
      className={`h-9 rounded-lg border px-3 text-xs font-bold inline-flex items-center gap-2 ${className}`}
    >
      <span
        className={`h-2 w-2 rounded-full ${active ? 'bg-emerald-500' : draft ? 'bg-amber-500' : 'bg-slate-400'}`}
      />
...
const PreviewMetric: React.FC<{
  ...
  <div className="rounded-lg border border-slate-200 bg-white p-3">
    ...
    <div className="mt-1 text-[10px] font-bold leading-snug text-slate-500">
      {label}
    </div>
  </div>
);
