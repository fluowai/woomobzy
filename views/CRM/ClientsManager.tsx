...
export default function ClientsManager() {
  ...
  const filtered = useMemo(() => {
    ...
    return clients.filter((c) => {
      if (
        roleFilter !== 'Todos' &&
        !c.roles?.some((r) => r.toLowerCase() === roleFilter.toLowerCase())
      )
        return false;
      if (!term) return true;
  ...
  return (
    <div className="workspace-page animate-fade-in space-y-5">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <button
          onClick={openNew}
          className="workspace-primary-action bg-primary text-white border-primary whitespace-nowrap"
        >
      ...
      <div className="workspace-card p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
          {['Todos', 'Proprietário', 'Inquilino', 'Comprador', 'Fiador'].map(
            (role) => (
              <button
                key={role}
              ...
              >
                ...
                {role === 'Todos'
                  ? `(${clients.length})`
                  : `(${clients.filter((c) => c.roles?.includes(role)).length})`}
              </button>
            )
          )}
        </div>
                    ...
                    >
                      ...
                      <td className="p-4 md:p-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={() =>
                              toast.info('Menu de opções do cliente em breve')
                            }
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Informações do cliente e definição de papéis.
                </p>
              </div>
              <button
                onClick={() => setIsModalOpen(false)}
                ...
                <div className="space-y-6">
                  <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                    <div className="flex items-center gap-3 text-slate-900 font-bold mb-4">
                      <FileText size={20} className="text-primary" /> Papel do
                      Cliente
                    </div>
                    ...
                    <div className="space-y-3">
                      {ROLE_OPTIONS.map((role) => (
                        <label
                          key={role.id}
                          className="flex items-start gap-3 p-3 rounded-xl border border-slate-200 bg-white hover:border-primary/50 hover:bg-primary/5 cursor-pointer transition-colors group"
                        >
