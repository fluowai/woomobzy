      ...
      >
        {(provided, snapshot) => (
          <section
            ...
            className={`flex h-full min-w-[17rem] flex-col overflow-hidden rounded-xl border border-slate-200 ${snapshot.isDraggingOver ? 'bg-indigo-50/70 ring-2 ring-indigo-200' : 'bg-slate-50'}`}
          >
            <header className="border-b border-slate-200 bg-white px-3 py-2.5">
              <div className="flex items-center justify-between gap-2">
                <span
                  className={`flex items-center gap-1.5 rounded-full px-2 py-1 text-[11px] font-bold uppercase ${stage.color}`}
                >
              ...
              {columnVgv && (
                <p className="mt-1.5 text-[10px] font-semibold text-slate-400">
                  VGV{' '}
                  <span className="font-bold text-slate-600">{columnVgv}</span>
                </p>
              )}
            </header>
            <div
            ...
            >
              ...
              >
                {virtualizer.getVirtualItems().map((virtualRow) => {
                  ...
                  return (
                    ...
                    >
                      <Draggable draggableId={l.id} index={virtualRow.index}>
                        {(dragProvided, dragSnapshot) =>
                          renderDraggable(l, dragProvided, dragSnapshot)
                        }
                      </Draggable>
                    </div>
                  );
              ...
              {hasMore && (
                <button
                  ...
                  className="mt-2 h-10 w-full rounded-xl border border-slate-200 bg-white text-[10px] font-bold uppercase text-slate-600 disabled:opacity-50"
                >
