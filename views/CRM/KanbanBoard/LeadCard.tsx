  ...
  ({
    ...
    const phoneLabel =
      phoneDigits.length > 4
        ? phoneDigits.slice(-4).padStart(8, '*')
        : lead.phone;
    const visibleTags = (lead.tags || [])
      .filter((t) => !String(t).startsWith('intent-'))
      .slice(0, 2);

    return (
      <div
        onClick={() => onOpen(lead)}
        className={`group relative cursor-pointer overflow-hidden rounded-xl border bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md ${selected ? 'ring-2 ring-indigo-500 border-indigo-400 bg-indigo-50/20' : `border-slate-200 ${sla.borderClass}`}`}
      >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0 pr-5">
                <h4
                  className="line-clamp-2 text-[13px] font-bold leading-snug text-slate-950"
                  title={displayName}
                >
                  {displayName}
                </h4>
              ...
              {scoreBadge && (
                <span
                  className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide ring-1 ${scoreBadge.bg}`}
                >
            ...
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              <span className="max-w-full truncate rounded-full bg-slate-50 px-2 py-1 text-[9px] font-bold uppercase tracking-wide text-slate-500 ring-1 ring-slate-200">
                {classification}
              </span>
              {visibleTags.map((tag) => (
                <span
                  key={tag}
                  className="max-w-[8rem] truncate rounded-full bg-indigo-50 px-2 py-1 text-[9px] font-bold uppercase tracking-wide text-indigo-600 ring-1 ring-indigo-100"
                >
                  {tag}
                </span>
              ))}
              ...
              <div className="mt-2 flex items-center gap-2 rounded-lg border border-slate-100 bg-slate-50 px-2.5 py-2">
                ...
                {Number(lead.property.price) > 0 && (
                  <span className="shrink-0 text-[10px] font-bold text-emerald-700">
                    {Number(lead.property.price).toLocaleString('pt-BR', {
                      style: 'currency',
                      currency: 'BRL',
                      notation: 'compact',
            ...
            <div className="mt-3 flex items-center justify-between gap-2 border-t border-slate-100 pt-2">
              <div className="flex min-w-0 items-center gap-1.5">
                {sla.label ? (
                  <span
                    className={`inline-flex items-center gap-1 rounded-full bg-white px-2 py-1 text-[10px] font-bold ${sla.labelClass}`}
                  >
                ...
                {lead.next_visit_at && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2 py-1 text-[10px] font-bold text-blue-600">
                ...
                {lead.ai_next_action && (
                  <span
                    ...
                    className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-indigo-50 text-indigo-500"
                  >
                    <Sparkles size={10} />
                  </span>
                )}
