...
const isImplicitTlsPort = (port: number, implicitTlsPort: number) =>
  Number(port) === implicitTlsPort;
...
const folderForApi = (folder: FolderKey) => {
  if (folder === 'starred' || folder === 'snoozed' || folder === 'all')
    return 'inbox';
...
const EmailCenter: React.FC = () => {
  ...
  const [accountForm, setAccountForm] =
    useState<ConnectEmailPayload>(blankAccount);
  ...
  const linkedLead = useMemo(() => {
    const found = selectedEmail?.lead_id
      ? leads.find((lead) => lead.id === selectedEmail.lead_id)
      : null;
  ...
  const todayCount = pendingAgenda.filter(
    (item) =>
  ...
  const loadEmails = async () => {
    try {
      const data = await emailService.listEmails(
        folderForApi(folder),
        1,
        search
      );
  ...
  const linkSelectedLead = async (leadId: string) => {
    if (!selectedEmail) return;
    await emailService.updateEmail(selectedEmail.id, {
      lead_id: leadId || null,
    });
            ...
            <div className="ml-auto flex items-center gap-4 text-xs font-medium text-slate-600">
              <span>
                {displayEmails.length
                  ? `1-${displayEmails.length} de ${displayEmails.length}`
                  : '0 de 0'}
              </span>
              <ChevronLeft size={18} />
          ...
          {view === 'agenda' ? (
            ...
            <>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto px-8 py-6">
                <div className="mb-6 flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h1 className="mb-0 text-xl font-medium text-slate-950">
                ...
                <MessageBlock
                  message={sortedThread[0] || selectedEmail}
                  first
        ...
        <Modal title="Novo email" onClose={() => setComposeOpen(false)}>
          <form onSubmit={sendNewEmail} className="space-y-3">
            ...
            <select
              ...
              onChange={(e) =>
                setCompose({ ...compose, lead_id: e.target.value })
              }
            >
            ...
            <textarea
              ...
              onChange={(e) =>
                setCompose({ ...compose, body_html: e.target.value })
              }
            />
...
const EmailRow: React.FC<{
  ...
  >
    ...
    <Star
      size={16}
      className={`mt-1 ${email.is_starred ? 'fill-yellow-400 text-yellow-400' : 'text-slate-400'}`}
    />
    <div className="min-w-0">
      <div className="flex items-center gap-2">
        <p
          className={`truncate text-sm ${email.is_read ? 'font-semibold text-slate-800' : 'font-bold text-slate-950'}`}
        >
        ...
        {email.label ? (
          <span
            className={`rounded-md px-2 py-0.5 text-[10px] font-bold ${email.labelColor}`}
          >
            {email.label}
          </span>
        ) : null}
      ...
      <p className="mt-1 truncate text-sm font-medium text-slate-500">
        {cleanSnippet(email.preview || email.body_text || '') || 'Sem preview'}
      </p>
    </div>
    <div className="flex flex-col items-end gap-2">
      <span
        className={`text-xs font-bold ${email.is_read ? 'text-slate-500' : 'text-emerald-600'}`}
      >
        {email.timeLabel || formatDateTime(email.date).slice(0, 5)}
      </span>
  ...
  return (
    <article>
      <div className="mb-4 flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mb-0 mt-1 text-xs font-medium text-slate-500">
              para {message.to_email?.join(', ') || 'mim'}{' '}
        ...
        <div className="flex shrink-0 items-center gap-4 text-slate-600">
          <span className="text-xs font-medium text-slate-500">
            {message.timeLabel || formatDateTime(message.date)}
          </span>
          <Star size={17} />
...
const ActionButton: React.FC<{
  ...
  <button
    onClick={onClick}
    className="flex h-10 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50"
  >
    <Icon size={17} />
    {label}
  </button>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
      {selectedEmail ? (
        <div className="mb-5 flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mb-0 mt-1 truncate text-xs font-medium text-slate-500">
              {linkedLead?.email || selectedEmail.from_email}
            </p>
            {linkedLead?.phone ? (
      ...
      {linkedLead ? (
        <div className="space-y-4 border-t border-slate-200 py-5">
          {linkedLead.source ? (
            <InfoRow label="Origem" value={linkedLead.source} />
          ) : null}
          {linkedLead.status ? (
            <InfoRow label="Etapa do funil" value={linkedLead.status} badge />
          ) : null}
          {linkedLead.email ? (
            <InfoRow label="E-mail" value={linkedLead.email} />
          ) : null}
          {linkedLead.phone ? (
            <InfoRow label="Telefone" value={linkedLead.phone} />
          ) : null}
          {linkedLead.notes ? (
            <InfoRow label="Observacoes" value={linkedLead.notes} multiline />
          ) : null}
        </div>
      ) : (
      ...
      <div className="border-t border-slate-200 py-5">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="mb-0 text-sm font-bold text-slate-900">
            Ultimas interacoes
          </h3>
          {pendingCount ? (
        ...
        {agenda.length ? (
          ...
          <p className="mb-0 text-xs font-semibold text-slate-500">
            Nenhuma interacao real registrada.
          </p>
        )}
      </div>
...
const InfoRow: React.FC<{
  ...
  avatar?: boolean;
  multiline?: boolean;
}> = ({ label, value, badge, avatar, multiline }) => (
  <div
    className={`grid grid-cols-[92px_1fr] gap-3 text-xs ${multiline ? 'items-start' : 'items-center'}`}
  >
    <span className="font-semibold text-slate-500">{label}</span>
    <span
      className={`font-semibold text-slate-800 ${multiline ? 'leading-relaxed' : ''}`}
    >
      {badge ? (
...
const Interaction: React.FC<{
  ...
  subtitle: string;
  date: string;
}> = ({ icon: Icon, color, title, subtitle, date }) => (
  <div className="mb-4 flex items-start gap-3">
    ...
    <div className="min-w-0 flex-1">
      ...
      {subtitle ? (
        <p className="mb-0 mt-1 truncate text-[11px] font-medium text-slate-500">
          {subtitle}
        </p>
      ) : null}
    </div>
    <span className="w-24 shrink-0 text-right text-[11px] font-medium text-slate-500">
      {date}
    </span>
  </div>
);

const SideAction: React.FC<{ icon: React.ElementType; label: string }> = ({
  icon: Icon,
  label,
}) => (
...
const AgendaPanel: React.FC<{ agenda: EmailAgendaActivity[] }> = ({
  agenda,
}) => (
  <div className="flex h-full flex-col">
    <div className="border-b border-slate-200 bg-white p-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="mb-0 text-xl font-bold text-slate-950">
            Diario de bordo
          </h1>
          <p className="mb-0 mt-1 text-xs font-semibold text-slate-500">
            Atividades criadas pelo agente a partir dos emails dos leads.
          </p>
        </div>
      </div>
    </div>
    <div className="grid flex-1 grid-cols-1 gap-4 overflow-y-auto p-5 xl:grid-cols-2">
      ...
      {!agenda.length ? (
        <EmptyState icon={Calendar} text="Nenhuma atividade de email." />
      ) : null}
    </div>
  </div>
);
  ...
  <div className="modal-overlay">
    <div className="modal-content max-w-2xl">
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <button
          onClick={onClose}
          className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:text-slate-900"
        >
          <X size={18} />
        </button>
      </div>
