  ...
  const handleNext = async () => {
    if (step === 1) {
      if (
        ...
        setError(
          'Preencha todos os campos obrigatórios e escolha um template.'
        );
        return;
      }
    ...
    <div className="animate-fade-in-slide space-y-6">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-slate-900">
          O Perfil da sua Imobiliária
        </h2>
        <p className="text-slate-500 mt-2">
          Vamos preparar a sua fundação digital.
        </p>
      </div>
  ...
  const renderStep2 = () => (
    <div className="animate-fade-in-slide space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 mt-2">
          Conecte o "cérebro" que vai atender seus clientes 24h por dia.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {['openai', 'gemini'].map((llm) => (
          <button
            ...
            className={`p-4 rounded-xl border-2 flex items-center justify-center gap-2 transition-all ${formData.llmProvider === llm ? 'border-blue-500 bg-blue-50' : 'border-slate-100 bg-white hover:border-slate-200'}`}
          >
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">
          Sua API Key (Opcional por agora)
        </label>
        <input
          type="password"
          value={formData.apiKey}
          onChange={(e) => update('apiKey', e.target.value)}
          className="w-full px-4 py-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:border-blue-500 transition-colors font-mono text-sm"
          placeholder="sk-..."
        />
        <p className="text-xs text-slate-400 mt-2">
          Você pode pular e configurar isso mais tarde no painel.
        </p>
      </div>
    </div>
  );
  ...
  const renderStep3 = () => (
    <div className="animate-fade-in-slide space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 mt-2">
          Onde a mágica do atendimento acontece.
        </p>
      </div>
      ...
      <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200 flex flex-col items-center justify-center text-center space-y-4">
        <div className="w-48 h-48 bg-white border-2 border-dashed border-slate-300 rounded-2xl flex items-center justify-center">
          <p className="text-slate-400 font-medium text-sm px-4">
            O QR Code aparecerá no seu painel administrativo.
          </p>
        </div>
        <p className="text-sm text-slate-600">
          Conectaremos o WhatsApp diretamente por lá para maior segurança.
        </p>
      </div>

      <div>
        <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">
          Mensagem de Saudação da IA
        </label>
        <textarea
          value={formData.welcomeMessage}
          onChange={(e) => update('welcomeMessage', e.target.value)}
          className="w-full px-4 py-3 bg-white rounded-xl border border-slate-200 outline-none focus:border-blue-500 transition-colors resize-none h-24"
        />
      </div>
    </div>
  );
  ...
  const renderStep4 = () => (
    <div className="animate-fade-in-slide space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 mt-2">
          Convide corretores ou sócios para colaborar com você. (Opcional)
        </p>
      </div>
          ...
          <div key={idx}>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:border-indigo-500 transition-colors"
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 selection:bg-blue-100">
      <div className="w-full max-w-2xl">
        ...
        {/* Content Box */}
        <div
          className={`bg-white rounded-[2rem] shadow-2xl shadow-slate-900/5 p-8 md:p-10 border border-slate-100 ${step === 5 ? 'border-emerald-100 bg-gradient-to-b from-emerald-50/50 to-white' : ''}`}
        >
          {error && (
          ...
          {step < 5 && (
            <div className="mt-10 flex items-center justify-between pt-6 border-t border-slate-100">
              <button
                ...
                className={`px-6 py-3 font-bold text-slate-500 hover:text-slate-800 transition-colors ${step === 1 ? 'opacity-0 pointer-events-none' : ''}`}
              >
                Voltar
              </button>
