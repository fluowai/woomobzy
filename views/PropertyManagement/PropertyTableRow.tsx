...
const PropertyTableRow: React.FC<PropertyTableRowProps> = ({
  property, isRural, portalPublishing, getPropertySummary, getPortalPublishes, onDelete, onPortalPublish, onPortalUnpublish,
}) => {
    ...
    <tr className="hover:bg-slate-50/50 transition-colors">
      <td className="px-4 sm:px-6 py-3 sm:py-4">
        <div className="flex items-center gap-3">
          <img src={property.images?.[0] || 'https://via.placeholder.com/400x300?text=Sem+Foto'} className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg object-cover shrink-0" />
          <div className="min-w-0">
            <p className="text-sm font-bold text-slate-900 truncate">{property.title}</p>
            <p className="text-xs text-slate-400">ID: {property.id?.slice(0, 8)} • {(property as any).source === 'orulo' ? 'Órulo' : getPropertySummary(property).join(' • ')}</p>
          </div>
        </div>
      </td>
      ...
      <td className="px-4 sm:px-6 py-3 sm:py-4">
        <div className="flex items-center gap-1 sm:gap-2">
          <button onClick={() => navigate(`${property.id}`)} className="p-1.5 text-slate-400 hover:text-indigo-600 transition-colors" title="Editar"><Edit3 size={16} /></button>
          <button onClick={() => onDelete(property.id!)} className="p-1.5 text-slate-400 hover:text-red-500 transition-colors" title="Excluir"><Trash2 size={16} /></button>
