...
const PublicLandingPage: React.FC<PublicLandingPageProps> = ({ forceSlug, forceComingSoon }) => {
  ...
  const loadLandingPage = async (slug: string) => {
    try {
      ...
      if (targetPageSlug) {
        ...
        query = query.in('slug', ['home', 'inicio', 'index', 'main', 'site']).limit(1);
      }
  ...
  const renderBlock = (block: any) => {
    ...
    switch (block.type) {
      case BlockType.HEADER: return <HeaderBlock config={block.config} theme={theme} />;
      case BlockType.FOOTER: return <FooterBlock config={block.config} theme={theme} />;
      case BlockType.HERO: return <HeroBlock config={block.config} theme={theme} />;
      case BlockType.PROPERTY_GRID: return <PropertyGridBlock config={block.config} theme={theme} />;
      case BlockType.TEXT: return <TextBlock config={block.config} theme={theme} />;
      case BlockType.FORM: return <FormBlock config={block.config} theme={theme} />;
      case BlockType.CTA: return <CTABlock config={block.config} theme={theme} />;
      default: return null;
    }
  };
  ...
  if ((forceComingSoon || !isLive) && !isSiteOwner) {
    ...
    const agencyName = settings?.agency_name || settings?.agencyName || organization?.name || 'Imobiliária';
    return <ComingSoon organizationId={organization?.id || ''} agencyName={agencyName} />;
  }

  if (showLogin && organization) return <SettingsProvider organizationId={organization.id}><Login /></SettingsProvider>;
  if (showMainSite && organization) return <SettingsProvider organizationId={organization.id}><MainLandingPage organizationId={organization.id} /></SettingsProvider>;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center max-w-md px-4">
          ...
          <p className="text-xl text-gray-600 mb-6">{error || 'Página não encontrada'}</p>
          <a href="/" className="inline-block px-8 py-3 bg-indigo-600 text-white rounded-xl shadow-lg hover:bg-indigo-700 transition-all">Voltar ao Início</a>
        </div>
      </div>
    );
  }
    ...
    <div style={{
      ...
      {!isLive && isSiteOwner && (
        <div className="bg-indigo-600 text-white px-4 py-2 text-center text-xs font-bold sticky top-0 z-[100] flex items-center justify-center gap-2">
          ...
          MODO MANUTENÇÃO ATIVO: Você visualiza o site real por ser Administrador.
        </div>
      )}
      <div className="flex-1">
        {page.blocks.map((block) => (
          <div key={block.id} className={getContainerClass(block.containerWidth)}>
            {renderBlock(block)}
          </div>
        ))}
      </div>
