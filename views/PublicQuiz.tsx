      ...
      <div className="grid min-h-screen lg:grid-cols-[42%_58%]">
        <aside className="relative hidden overflow-hidden lg:block">
          ...
          <div className="absolute inset-x-0 bottom-0 p-10 text-white xl:p-14">
            <p className="text-xs font-bold uppercase tracking-[0.28em]" style={{ color: primary }}>{selectionLabel}</p>
            ...
            <div className="w-full max-w-2xl">
              {result ? (
                <div className="text-center">
                  ...
                  <p className="mt-6 text-xs font-bold uppercase tracking-[0.24em]" style={{ color: primary }}>Resultado da analise</p>
                  ...
                  {result.qualified && result.whatsapp_url ? (
                    <div className="mt-8">
                      <a href={result.whatsapp_url} className="inline-flex h-12 items-center justify-center gap-2 px-6 text-sm font-bold text-white shadow-lg shadow-orange-600/20" style={{ backgroundColor: primary }}>
                    ...
                    <a href="/" className="mt-8 inline-flex h-12 items-center justify-center gap-2 border border-[#dcd5ce] px-6 text-sm font-bold text-[#242424]">Voltar ao site</a>
                  )}
                </div>
              ) : step === -1 ? (
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.24em]" style={{ color: primary }}>{qualificationLabel}</p>
                  ...
                  <button type="button" disabled={!canContinueContact} onClick={next} className="mt-7 inline-flex h-12 w-full items-center justify-center gap-2 px-6 text-sm font-bold text-white shadow-lg shadow-orange-600/20 disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto" style={{ backgroundColor: primary }}>
                ...
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.24em]" style={{ color: primary }}>Pergunta {step + 1} de {questions.length}</p>
                  ...
                  <div className="mt-7 flex items-center justify-between gap-4">
                    <button type="button" onClick={() => setStep((value) => value - 1)} className="inline-flex h-11 items-center gap-2 px-1 text-sm font-bold text-[#6d7178] hover:text-[#242424]"><ArrowLeft size={18} /> Voltar</button>
                    {step === questions.length - 1 ? (
                      <button type="button" disabled={!canContinueQuestion || submitting} onClick={submit} className="inline-flex h-12 items-center justify-center gap-2 px-6 text-sm font-bold text-white disabled:opacity-40" style={{ backgroundColor: primary }}>
                        {submitting ? <Loader2 className="animate-spin" size={18} /> : <CheckCircle2 size={18} />} Verificar meu perfil
                      </button>
                    ) : (
                      <button type="button" disabled={!canContinueQuestion} onClick={next} className="inline-flex h-12 items-center justify-center gap-2 bg-[#242424] px-6 text-sm font-bold text-white disabled:opacity-40">Continuar <ArrowRight size={18} /></button>
                    )}
                  </div>
                </div>
              ) : null}
