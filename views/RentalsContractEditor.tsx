...
export function RentalsContractEditor({ leaseId, onClose }: Props) {
  ...
  const [chatMessages, setChatMessages] = useState<{ role: 'ai' | 'user'; text: string; isAction?: boolean; isHighlight?: boolean }[]>([
    { role: 'ai', text: 'Olá! Sou seu assistente jurídico alimentado pelas maiores IAs do mercado (OpenAI, Gemini e Groq). Como posso ajudar no seu contrato?' }
  ]);
  ...
  const handleInputChange = (field: keyof typeof contractData, value: string) => {
...
`;
      ...
      try {
        ...
        for (const key in updatedData) {
          if (updatedData[key as keyof typeof contractData] !== contractData[key as keyof typeof contractData]) {
        ...
        if (changedFields.length > 0) {
          ...
          setChatMessages(prev => [...prev, { role: 'ai', text: 'Entendi, mas não encontrei campos exatos para atualizar com essa instrução ou os valores já estão iguais.' }]);
        }
      ...
      setChatMessages(prev => [...prev, { role: 'ai', text: 'Ocorreu um erro ao processar sua solicitação.' }]);
    } finally {
      ...
      <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 shrink-0 shadow-sm z-20">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center text-sm text-gray-500">
            <span className="hover:text-gray-900 cursor-pointer transition-colors" onClick={onClose}>Contratos</span>
            <span className="mx-2">/</span>
            <span className="font-semibold text-gray-900">Editor 360° com IA</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button onClick={onClose} className="ml-2 p-2 text-gray-500 hover:bg-red-50 hover:text-red-600 rounded-full transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-72 bg-white border-r border-gray-200 flex flex-col overflow-y-auto shrink-0 z-10 shadow-sm">
          <div className="p-6">
            ...
            <p className="text-sm text-gray-500 mb-6">Escolha a origem dos dados</p>
            ...
            <div className="space-y-3 mb-10">
              {importOptions.map((opt) => (
                ...
                >
                  <div className={`mt-0.5 p-2 rounded-lg ${activeImport === opt.id ? 'bg-emerald-500 text-white shadow-sm' : 'bg-gray-100 text-gray-500'}`}>
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-900 mb-4 uppercase tracking-widest text-xs">Etapas do processo</h3>
              <div className="space-y-6 relative before:absolute before:inset-0 before:ml-3 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-200 before:to-transparent">
                ...
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  ...
                  <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2.5rem)] ml-3 md:ml-0">
                    <h4 className="font-semibold text-sm text-gray-900">Base gerada</h4>
                    <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Modelo Padrão</p>
                  </div>
                </div>
                ...
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  ...
                  <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2.5rem)] ml-3 md:ml-0">
                    ...
                    <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">100% Inteligência</p>
                  </div>
                </div>
                ...
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                  ...
                  <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2.5rem)] ml-3 md:ml-0">
                    <h4 className="font-semibold text-sm text-emerald-700">Edição Dinâmica</h4>
                    <p className="text-[10px] uppercase font-bold text-emerald-600/80 tracking-wider">Edite ou peça à IA</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="flex-1 bg-slate-100 flex flex-col relative overflow-hidden">
          ...
          <div className="px-8 pt-4 pb-2 shrink-0">
            <div className="bg-emerald-50 border border-emerald-200 shadow-sm rounded-xl p-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-emerald-800 font-bold text-sm">Pronto para Edição!</h3>
                  <p className="text-emerald-600/80 text-xs">Os dados estão sincronizados com a barra lateral. Edite livremente.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="px-8 pb-4 shrink-0 flex items-center justify-between">
            <div className="flex items-center gap-2 bg-white rounded-lg shadow-sm border border-gray-200 p-1">
              <button className="p-1.5 text-gray-500 hover:bg-emerald-50 hover:text-emerald-600 rounded-md transition-colors"><Edit2 className="w-4 h-4" /></button>
              <div className="w-px h-4 bg-gray-200"></div>
              <button className="p-1.5 text-gray-500 hover:bg-emerald-50 hover:text-emerald-600 rounded-md transition-colors"><FileText className="w-4 h-4" /></button>
            </div>
            ...
            <div className="flex items-center gap-1 bg-white rounded-lg shadow-sm border border-gray-200 p-1">
              ...
              <button className="p-1.5 text-gray-500 hover:bg-gray-100 rounded-md"><ZoomOut className="w-4 h-4" /></button>
          ...
          <div className="flex-1 overflow-y-auto px-8 pb-16 pt-2">
            <div className="max-w-[800px] mx-auto bg-white shadow-xl min-h-[1100px] border border-gray-200 p-16 font-serif text-[15px] leading-relaxed text-gray-800 transition-all">
              <h1 className="text-2xl font-bold text-center text-gray-900 mb-10">CONTRATO DE LOCAÇÃO<br/>DE IMÓVEL URBANO</h1>
              
              <p className="mb-6 text-justify">Pelo presente instrumento particular de locação de imóvel, as partes abaixo qualificadas têm entre si justo e contratado o seguinte:</p>

              <h2 className="font-bold mb-4 uppercase text-gray-900">Cláusula 1ª – Das Partes</h2>
              
              <p className="mb-4 text-justify">
                <strong>LOCADOR:</strong> <span className="bg-emerald-100/80 text-emerald-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.locador_nome}</span>, residente e domiciliado no Brasil, inscrito no CPF sob o nº <span className="bg-emerald-100/80 text-emerald-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.locador_cpf}</span>, podendo ser contatado via telefone <span className="bg-emerald-100/80 text-emerald-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.locador_telefone}</span> ou pelo correio eletrônico <span className="bg-emerald-100/80 text-emerald-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.locador_email}</span>.
              </p>

              <p className="mb-8 text-justify">
                <strong>LOCATÁRIO:</strong> <span className="bg-blue-100/80 text-blue-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.locatario_nome}</span>, residente e domiciliado no Brasil, inscrito no CPF sob o nº <span className="bg-blue-100/80 text-blue-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.locatario_cpf}</span>.
              </p>

              <h2 className="font-bold mb-4 uppercase text-gray-900">Cláusula 2ª – Do Imóvel</h2>

              <p className="mb-8 text-justify">
                O LOCADOR dá em locação ao LOCATÁRIO o imóvel assim descrito:
                Localizado na <span className="bg-amber-100/80 text-amber-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.imovel_endereco}</span>, na cidade de <span className="bg-amber-100/80 text-amber-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.imovel_cidade}</span>, CEP <span className="bg-amber-100/80 text-amber-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.imovel_cep}</span>, para o fim exclusivo de locação residencial.
              </p>

              <h2 className="font-bold mb-4 uppercase text-gray-900">Cláusula 3ª – Do Valor e Pagamento</h2>

              <p className="mb-4 text-justify">
                O aluguel mensal ajustado é de <span className="bg-purple-100/80 text-purple-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">R$ {contractData.aluguel_valor}</span>, vencível e pagável impreterivelmente até o dia <span className="bg-pink-100/80 text-pink-900 font-semibold px-1.5 py-0.5 rounded shadow-sm transition-all">{contractData.aluguel_vencimento}</span> de cada mês subsequente ao vencido.
              </p>

              <p className="mb-8 text-justify">
                O pagamento será realizado mediante depósito bancário ou PIX na conta indicada pelo LOCADOR, valendo o comprovante como recibo para todos os fins.
              </p>
            </div>
          </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 py-3 px-6 flex items-center justify-center gap-6 text-[10px] font-bold uppercase tracking-widest text-gray-500 shadow-[0_-4px_15px_rgba(0,0,0,0.05)] z-20">
            ...
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-emerald-400 shadow-sm"></div> Locador</div>
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-blue-400 shadow-sm"></div> Locatário</div>
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-amber-400 shadow-sm"></div> Imóvel</div>
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-purple-400 shadow-sm"></div> Valores</div>
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-pink-400 shadow-sm"></div> Datas</div>
          </div>
        </section>
        ...
        <aside className="w-[420px] bg-white border-l border-gray-200 flex flex-col shrink-0 z-30 shadow-[-4px_0_15px_rgba(0,0,0,0.04)]">
          ...
          <div className="h-[55%] flex flex-col border-b border-gray-200 bg-gray-50/30">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-white shadow-sm z-10">
              <h2 className="font-bold text-gray-900 text-base">Campos do Documento</h2>
              <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100">Edição Ativa</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
              ...
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:border-emerald-200 transition-colors">
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-emerald-400"></div> Locador
                  </div>
                  <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandedSection === 'locador' ? 'rotate-180 text-emerald-600' : ''}`} />
                </button>
                {expandedSection === 'locador' && (
                  <div className="p-5 bg-white border-t border-gray-100 grid grid-cols-2 gap-4">
                    <div className="col-span-2">
                      <label className="text-[10px] uppercase font-bold text-gray-400 mb-1.5 block tracking-wider">Nome Completo</label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-400 transition-all">
                        ...
                        <input 
                          ...
                          onChange={(e) => handleInputChange('locador_nome', e.target.value)}
                          className="text-sm font-semibold text-gray-900 bg-transparent outline-none w-full"
                        />
                      </div>
                    </div>
                    <div>
                      <label className="text-[10px] uppercase font-bold text-gray-400 mb-1.5 block tracking-wider">CPF</label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-400 transition-all">
                        <CreditCard className="w-4 h-4 text-emerald-500 shrink-0" />
                        <input 
                          type="text" 
                          value={contractData.locador_cpf} 
                          onChange={(e) => handleInputChange('locador_cpf', e.target.value)}
                          className="text-sm font-semibold text-gray-900 bg-transparent outline-none w-full"
                        />
                      </div>
                    </div>
                    <div>
                      <label className="text-[10px] uppercase font-bold text-gray-400 mb-1.5 block tracking-wider">Telefone</label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-400 transition-all">
                        <Phone className="w-4 h-4 text-emerald-500 shrink-0" />
                        <input 
                          type="text" 
                          value={contractData.locador_telefone} 
                          onChange={(e) => handleInputChange('locador_telefone', e.target.value)}
                          className="text-sm font-semibold text-gray-900 bg-transparent outline-none w-full"
                        />
                      </div>
                    </div>
                    <div className="col-span-2">
                      <label className="text-[10px] uppercase font-bold text-gray-400 mb-1.5 block tracking-wider">E-mail</label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-400 transition-all">
                        <Mail className="w-4 h-4 text-emerald-500 shrink-0" />
                        <input 
                          type="text" 
                          value={contractData.locador_email} 
                          onChange={(e) => handleInputChange('locador_email', e.target.value)}
                          className="text-sm font-semibold text-gray-900 bg-transparent outline-none w-full"
                        />
                      </div>
                    </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:border-blue-200 transition-colors">
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-blue-400"></div> Locatário
                  </div>
                  <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandedSection === 'locatario' ? 'rotate-180 text-blue-600' : ''}`} />
                </button>
                     ...
                     <div className="col-span-2">
                      ...
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-400 transition-all">
                        ...
                        <input 
                          ...
                          onChange={(e) => handleInputChange('locatario_nome', e.target.value)}
                          className="text-sm font-semibold text-gray-900 bg-transparent outline-none w-full"
                        />
                      </div>
                    </div>
                    <div className="col-span-2">
                      ...
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-400 transition-all">
                        ...
                        <input 
                          ...
                          onChange={(e) => handleInputChange('locatario_cpf', e.target.value)}
                          className="text-sm font-semibold text-gray-900 bg-transparent outline-none w-full"
                        />
                      </div>
                    </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:border-amber-200 transition-colors">
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-amber-400"></div> Imóvel
                  </div>
                  <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandedSection === 'imovel' ? 'rotate-180 text-amber-600' : ''}`} />
                </button>
                {expandedSection === 'imovel' && (
                  <div className="p-5 bg-white border-t border-gray-100 grid grid-cols-2 gap-4">
                    <div className="col-span-2">
                      <label className="text-[10px] uppercase font-bold text-gray-400 mb-1.5 block tracking-wider">Endereço</label>
                    ...
                    <div>
                      <label className="text-[10px] uppercase font-bold text-gray-400 mb-1.5 block tracking-wider">Cidade/UF</label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-amber-500/20 focus-within:border-amber-400 transition-all">
                        <input 
                          type="text" 
                    ...
                    <div>
                      <label className="text-[10px] uppercase font-bold text-gray-400 mb-1.5 block tracking-wider">CEP</label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-amber-500/20 focus-within:border-amber-400 transition-all">
                        <input 
                          type="text" 
              ...
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:border-purple-200 transition-colors">
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-purple-400"></div> Condições
                  </div>
                  <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandedSection === 'condicoes' ? 'rotate-180 text-purple-600' : ''}`} />
                </button>
                {expandedSection === 'condicoes' && (
                  <div className="p-5 bg-white border-t border-gray-100 grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-[10px] uppercase font-bold text-gray-400 mb-1.5 block tracking-wider">Valor do Aluguel</label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-purple-500/20 focus-within:border-purple-400 transition-all">
                        ...
                        <input 
                          ...
                          onChange={(e) => handleInputChange('aluguel_valor', e.target.value)}
                          className="text-sm font-semibold text-gray-900 bg-transparent outline-none w-full"
                        />
                      </div>
                    </div>
                    <div>
                      <label className="text-[10px] uppercase font-bold text-gray-400 mb-1.5 block tracking-wider">Vencimento (Dia)</label>
                      <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2.5 bg-gray-50 focus-within:ring-2 focus-within:ring-pink-500/20 focus-within:border-pink-400 transition-all">
                        ...
                        <input 
                          ...
                          onChange={(e) => handleInputChange('aluguel_vencimento', e.target.value)}
                          className="text-sm font-semibold text-gray-900 bg-transparent outline-none w-full"
                        />
                      </div>
                    </div>
          ...
          <div className="flex-1 flex flex-col bg-white relative border-t-2 border-indigo-50">
            <div className="p-4 bg-gradient-to-r from-indigo-900 to-indigo-800 flex items-center justify-between shadow-sm z-10">
              <div className="flex items-center gap-2 text-white">
                ...
                <span className="px-1.5 py-0.5 bg-indigo-500 text-white text-[9px] font-bold uppercase rounded-md shadow-inner">Conectado</span>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5 custom-scrollbar bg-indigo-50/30">
              {chatMessages.map((msg, idx) => (
                <div key={idx} className={`flex items-start gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`p-3.5 rounded-2xl shadow-sm text-sm max-w-[85%] font-medium ${
                    msg.role === 'user' 
          ...
          <div className="p-5 bg-white border-t border-gray-200 shrink-0 shadow-[0_-10px_20px_rgba(0,0,0,0.03)] z-20">
            <h3 className="font-bold text-gray-900 mb-4 text-sm uppercase tracking-widest">Ações Finais</h3>
