...
const SystemSalesPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-[#07172a] selection:bg-emerald-100 selection:text-emerald-900 font-sans">
      ...
      <header className="sticky top-0 z-50 border-b border-emerald-900/10 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/login')} className="hidden text-sm font-bold text-slate-600 hover:text-emerald-600 md:block">
              Entrar
            </button>
            <button onClick={() => scrollToSection('demo-form')} className="rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-bold text-white shadow-md shadow-emerald-600/20 transition-all hover:-translate-y-0.5 hover:bg-emerald-700 hover:shadow-lg hover:shadow-emerald-600/30">
              Agendar Demo
            </button>
          </div>
        </div>
      ...
      <main>
        {/* NEW HERO LAYOUT: Centered Text & Big Image Below */}
        <section id="plataforma" className="relative pt-24 pb-32 text-center bg-gradient-to-b from-white to-slate-50">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(16,185,129,0.05)_0%,transparent_50%)]" />
          <div className="relative mx-auto max-w-5xl px-6">
            <div className="mx-auto mb-8 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-4 py-1.5 text-xs font-black uppercase tracking-wider text-emerald-700">
            ...
            <h1 className="text-5xl font-black leading-[1.1] tracking-tight text-slate-900 md:text-7xl">
              Imobiliárias que faturam usam <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-500">processos.</span>
            </h1>
            
            <p className="mx-auto mt-6 max-w-2xl text-lg font-medium text-slate-600 md:text-xl">
            ...
            <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
              <button onClick={() => scrollToSection('demo-form')} className="flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-8 text-base font-black text-white shadow-xl shadow-emerald-600/20 transition-all hover:scale-105 hover:bg-emerald-700 sm:w-auto">
                Ver na Prática <ArrowRight size={18} />
              </button>
              <button onClick={() => scrollToSection('beneficios')} className="flex h-14 w-full items-center justify-center gap-2 rounded-2xl border-2 border-slate-200 bg-white px-8 text-base font-bold text-slate-700 transition-all hover:border-emerald-200 hover:bg-emerald-50 sm:w-auto">
                Explorar Soluções
              </button>
            </div>

            <div className="mx-auto mt-20 max-w-5xl overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-2 shadow-2xl shadow-slate-900/10">
              <div className="rounded-[1.5rem] bg-slate-100 overflow-hidden border border-slate-100 relative">
                 ...
                 {/* Fake UI Overlay */}
                 <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-slate-900/80 to-transparent flex items-end p-8">
                    <div className="flex gap-4">
                      {metrics.map((m, i) => (
                        <div key={i} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 text-white">
