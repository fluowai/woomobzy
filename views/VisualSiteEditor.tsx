...
const VisualSiteEditor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const resolveSiteRoute = async () => {
      try {
        ...
        if (lpData) {
          setPageId(lpData.id);
          setCurrentTheme(lpData.theme_config || lpData.themeConfig || {});
        }
      } catch (error) {
  ...
  const handleApplyTemplate = async (templateId: string) => {
    ...
    if (!confirm('Deseja aplicar este modelo? A estrutura da página será substituída pelo novo modelo.')) return;
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        const extractedColors = await geminiService.extractColorsFromLogo(base64, file.type);
        ...
        if (orgId) {
          try {
            await supabase.from('site_settings').upsert({
             ...
             console.warn('Silent skip se a logo nao for suportada via base64 na DB local.', err);
          }
        }

        alert('✨ A IA detectou as cores da sua logo e o tema do site foi atualizado!');
        reloadIframe();
    ...
    <div className="flex flex-col h-screen bg-gray-900 text-white overflow-hidden">
      ...
      <div className="h-14 bg-gray-800 border-b border-gray-700 flex items-center justify-between px-4 shrink-0 z-10 relative shadow-md">
        ...
        <div className="flex items-center gap-3">
          {saving && <Loader2 className="animate-spin text-gray-400" size={16} />}
        ...
        >
          {isSidebarOpen && (
            <div className="flex flex-col h-full overflow-y-auto custom-scrollbar p-5 w-80">
              ...
              <div className="mb-8">
                ...
                <div className="bg-gray-700/50 border border-gray-600 rounded-xl p-4">
                  ...
                  <p className="text-xs text-gray-400 mb-4">
                    Faça upload da sua logo e a IA configurará as cores do seu site instantaneamente.
                  </p>
                  ...
                  {currentTheme && (
                     <div className="flex items-center gap-3 mb-4 justify-center">
                        <div className="flex flex-col items-center gap-1">
                          <div className="w-8 h-8 rounded-full shadow-lg border-2 border-gray-500" style={{ backgroundColor: currentTheme.primaryColor }} />
                          <span className="text-[10px] text-gray-400">Primária</span>
                        </div>
                        <div className="flex flex-col items-center gap-1">
                          <div className="w-8 h-8 rounded-full shadow-lg border-2 border-gray-500" style={{ backgroundColor: currentTheme.secondaryColor }} />
                          <span className="text-[10px] text-gray-400">Secundária</span>
                        </div>
                     </div>
                  )}
                  ...
                  <div className="relative">
                    ...
                    >
                      {uploadingLogo ? <Loader2 className="animate-spin" size={16}/> : <Sparkles size={16}/>}
                      {uploadingLogo ? 'Analisando cores...' : 'Subir Logo com IA'}
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                  <Layout size={16}/> Modelos Prontos ({window.location.pathname.includes('/rural') ? 'Rural' : 'Urbano'})
                </h3>
                
                <div className="space-y-3">
                  {LANDING_PAGE_TEMPLATES.filter(t => window.location.pathname.includes('/rural') ? t.category !== 'Urban' : t.category === 'Urban').slice(0, 5).map((template) => (
                    <div 
                      key={template.id}
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xl filter drop-shadow-md">{template.thumbnail}</span>
                        <div className="flex gap-1">
                          <div className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: template.themeConfig.primaryColor }} />
                          <div className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: template.themeConfig.secondaryColor }} />
                        </div>
                      </div>
