...
export default function CampaignDispatch({
  ...
  const total =
    campaign.contacts_summary.pending +
    campaign.contacts_summary.sent +
    campaign.contacts_summary.failed +
    campaign.contacts_summary.blacklisted;
  ...
  const pollProgress = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `/api/campaigns/${campaign.id}/dispatch/progress`,
        { headers }
      );
      if (res.ok) {
    ...
    <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <p className="text-white">
              {campaign.min_delay_seconds || 45}s –{' '}
              {campaign.max_delay_seconds || 180}s
            </p>
          </div>
          <div>
            <span className="text-gray-500 text-xs">Limite/dia</span>
            <p className="text-white">
              {campaign.daily_limit_per_instance || 50} por instância
            </p>
          </div>
          <div>
            <span className="text-gray-500 text-xs">Horário</span>
            <p className="text-white">
              {String(campaign.working_hours_start ?? 8).padStart(2, '0')}h –{' '}
              {String(campaign.working_hours_end ?? 20).padStart(2, '0')}h
            </p>
          </div>
          <div>
      ...
      {campaign.instances?.length === 0 && (
        <div className="mt-4 flex items-center gap-2 text-yellow-400 text-sm bg-yellow-900/20 rounded-lg p-3 border border-yellow-800/50">
          ...
          Atribua pelo menos uma instância WhatsApp na aba "Instâncias" antes de
          iniciar o disparo.
        </div>
      )}
    </div>
