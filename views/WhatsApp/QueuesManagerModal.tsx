...
export const QueuesManagerModal: React.FC<QueuesManagerModalProps> = ({ onClose }) => {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900">Filas de Atendimento</h2>
              <p className="text-xs font-medium text-slate-500">Organize os tickets de atendimento por departamento.</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <div className="space-y-3">
            {queues.map(queue => (
              <div key={queue.id} className="border border-slate-200 rounded-xl p-4 flex items-center justify-between hover:border-purple-200 transition bg-white">
                <div className="flex items-center gap-4">
                  <div className={`w-2 h-2 rounded-full ${queue.active ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                  <div>
                    <h4 className="text-sm font-bold text-slate-900">{queue.name}</h4>
          ...
          <div className="mt-8 bg-blue-50 border border-blue-100 rounded-xl p-4">
            <h4 className="text-sm font-bold text-blue-900 mb-1">Como funcionam as filas?</h4>
            <p className="text-xs font-medium text-blue-700 leading-relaxed">
              Quando a Inteligência Artificial não conseguir resolver uma solicitação ou o cliente pedir para falar com um humano, o atendimento será transferido para a fila apropriada. Os corretores disponíveis naquela fila poderão assumir o atendimento.
            </p>
          </div>
        </div>
      </div>
