...
const Chat: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'todos' | 'ativos' | 'automacao' | 'arquivados'>('todos');
    ...
    try {
      ...
      if (data.success) {
        ...
        if (!selectedInstance && data.instances?.length > 0) {
          const first = data.instances.find((i: Instance) => 
            (i.status === 'connected' || i.status === 'connecting') && i.socket_alive !== false
          );
          setSelectedInstance(first || data.instances[0]);
        }
      }
    } catch (err) { setError('Falha ao carregar instâncias'); } finally { setLoading(false); }
  ...
  const sendMessage = async () => {
    ...
    } catch (err) { setError('Erro ao enviar'); } finally { setSending(false); }
  };
  ...
  const filteredChats = chats.filter(chat => {
    ...
    if (activeTab === 'automacao') return matchesSearch && chat.jid.includes('bot'); // Simplificado
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-full bg-[#0F172A]">
      ...
      <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px]">Iniciando Painel Rural...</p>
    </div>
  );
  ...
  return (
    <div className="flex h-full bg-[#0F172A] text-slate-100 overflow-hidden font-sans">
      ...
      {/* ────────────────────────────────────────────────────────────────
          ...
          ──────────────────────────────────────────────────────────────── */}
      <div className={`${selectedChat ? 'hidden lg:flex' : 'flex'} flex-col w-full lg:w-[400px] bg-[#1E293B]/40 border-r border-slate-800 shrink-0`}>
            ...
            >
              ...
              <div className="flex-1 text-left min-w-0">
                <div className="flex justify-between items-start mb-1">
                  <h3 className={`font-bold truncate text-sm tracking-tight ${selectedChat?.id === chat.id ? 'text-emerald-400' : 'text-slate-100'}`}>
                ...
                <div className="flex items-center gap-2 mb-1">
                  <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700`}>
                    {chat.lead_info?.classification || 'Lead'}
                  </span>
                  <p className="text-xs text-slate-500 truncate font-medium">
                    {chat.jid.endsWith('@g.us') ? 'Grupo Ativo' : 'Atendimento Direto'}
                  </p>
                </div>
              </div>
      ...
      {/* ────────────────────────────────────────────────────────────────
          ...
          ──────────────────────────────────────────────────────────────── */}
      <div className={`${!selectedChat ? 'hidden lg:flex' : 'flex'} flex-1 flex-col bg-slate-900 relative`}>
        {selectedChat ? (
          <>
            {/* Header */}
            <header className="h-[90px] px-8 flex items-center justify-between border-b border-slate-800/50 bg-[#0F172A]/80 backdrop-blur-md z-10 shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setShowCRM(!showCRM)} className={`p-3 rounded-2xl transition-all shadow-xl ${showCRM ? 'bg-emerald-500 text-white' : 'bg-slate-800 text-slate-400'}`}>
                  <Info size={20} />
                </button>
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-[#0F172A] pattern-dots">
              <div className="max-w-4xl mx-auto space-y-8">
                {messages.map((msg, i) => (
                  <div key={msg.id} className={`flex ${msg.from_me ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                    <div className={`group relative max-w-[75%] px-6 py-4 rounded-[32px] shadow-2xl transition-all ${
                      msg.from_me 
                        ? 'bg-emerald-600 text-white rounded-tr-none' 
                        : 'bg-slate-800 text-slate-100 rounded-tl-none border border-slate-700/50'
                    }`}>
                      <p className="text-[15px] font-medium leading-relaxed mb-2">{msg.content}</p>
                      <div className={`flex items-center gap-2 justify-end text-[9px] font-black uppercase tracking-widest ${msg.from_me ? 'text-emerald-200' : 'text-slate-500'}`}>
                        {formatTime(msg.timestamp)}
                        {msg.from_me && <CheckCheck size={12} className={msg.status === 'read' ? 'text-white' : 'opacity-40'} />}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="p-6 bg-[#0F172A] border-t border-slate-800/80">
              <div className="max-w-4xl mx-auto flex items-center gap-3 bg-slate-800/50 p-2 rounded-[32px] border border-slate-700/50 focus-within:ring-4 focus-within:ring-emerald-500/10 transition-all">
                <button className="p-3 text-slate-500 hover:text-emerald-500 transition-colors"><Paperclip size={20} /></button>
                <button className="p-3 text-slate-500 hover:text-emerald-500 transition-colors hidden sm:block"><Smile size={20} /></button>
                <input 
                  type="text" 
                  value={newMessage}
                ...
                />
                <button className="p-3 text-slate-500 hover:text-emerald-500 transition-colors hidden sm:block"><Mic size={20} /></button>
                <button 
                  onClick={sendMessage}
                  disabled={!newMessage.trim() || sending}
                  className={`p-4 rounded-full transition-all shadow-xl shadow-emerald-500/20 ${
                    newMessage.trim() && !sending ? 'bg-emerald-500 text-white hover:scale-105' : 'bg-slate-700 text-slate-500'
                  }`}
                >
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center opacity-40">
            ...
            <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-4">Painel de Negócios</h2>
            <p className="max-w-xs text-sm font-bold uppercase tracking-widest leading-loose">Selecione uma oportunidade à esquerda para iniciar o atendimento</p>
          </div>
        )}
      </div>
        ...
        <div className="hidden xl:flex flex-col w-[400px] bg-[#1E293B]/60 border-l border-slate-800 p-8 overflow-y-auto custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-4 mb-10">
            <button className="flex flex-col items-center gap-2 p-4 bg-slate-900/50 rounded-3xl border border-slate-800 hover:border-emerald-500/40 transition-all group">
              <Edit3 size={18} className="text-slate-500 group-hover:text-emerald-500" />
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Ver Ficha</span>
            </button>
            <button className="flex flex-col items-center gap-2 p-4 bg-slate-900/50 rounded-3xl border border-slate-800 hover:border-emerald-500/40 transition-all group">
              <ExternalLink size={18} className="text-slate-500 group-hover:text-emerald-500" />
          ...
          <div className="space-y-8">
            ...
            <section>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h5 className="text-xs font-black uppercase tracking-widest text-white">Informações Base</h5>
              </div>
              <div className="bg-slate-900/50 rounded-3xl border border-slate-800 p-6 space-y-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-200">{selectedChat.lead_info?.email || 'un@invest.com'}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-500 uppercase mb-1">Origem</p>
                  <p className="text-sm font-medium text-slate-200">Campanha Google Ads - Mato Grosso</p>
                </div>
              </div>
            </section>
            ...
            <section>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h5 className="text-xs font-black uppercase tracking-widest text-white">Histórico e Score</h5>
              </div>
              <div className="bg-slate-900/50 rounded-3xl border border-slate-800 p-6 space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Interações</span>
                  <span className="text-xs font-black text-emerald-500">14 total</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Tempo de Resp.</span>
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h5 className="text-xs font-black uppercase tracking-widest text-white">Marcadores</h5>
                </div>
                <button className="text-emerald-500 hover:rotate-90 transition-transform"><Plus size={16}/></button>
              </div>
              <div className="flex flex-wrap gap-2">
                {['Mato Grosso', 'Gado Corte', 'Investidor', 'MT'].map(t => (
                  <span key={t} className="px-3 py-1 bg-slate-800 rounded-lg text-[10px] font-bold text-slate-400 border border-slate-700">{t}</span>
                ))}
              </div>
            </section>
            ...
            {/* Notas Rápidas */}
            <section>
              <h5 className="text-xs font-black uppercase tracking-widest text-white mb-4">Notas do Consultor</h5>
              <textarea 
