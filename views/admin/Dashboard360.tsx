      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase italic">
            Bem-vindo à sua <span className="text-blue-600">Máquina 360</span>,{' '}
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="h-full bg-white p-8 rounded-[32px] shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link
                to="../chat"
                className="text-xs font-black uppercase text-emerald-600 hover:underline"
              >
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-4 bg-slate-50 rounded-[24px] hover:bg-white hover:shadow-md transition-all border border-transparent hover:border-slate-100 group"
                >
                  <div className="flex items-center gap-4">
          ...
          <div className="bg-slate-900 p-8 rounded-[32px] text-white overflow-hidden relative border-none">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-8">
                ...
                <span className="px-2 py-1 bg-blue-500/20 text-blue-300 text-[9px] font-black uppercase tracking-widest rounded">
                  Asaas Integrado
                </span>
              </div>
              <p className="text-blue-200/60 text-[10px] font-black uppercase tracking-widest mb-1">
                Cobrabilidade este mês
              </p>
              <h4 className="text-3xl font-black italic tracking-tighter mb-6">
                R$ 48.250,00
              </h4>
...
const StatCard = ({ icon: Icon, label, value, change, color }: any) => {
  ...
  return (
    <div className="bg-white p-7 rounded-[32px] shadow-sm border border-slate-100 hover:shadow-xl transition-all group">
      ...
      <p className="text-3xl font-black text-slate-900 italic tracking-tighter group-hover:scale-105 transition-transform origin-left">
        {value}
      </p>
    </div>
  );
...
const ActionCard = ({ icon: Icon, label, desc, color }: any) => {
  ...
  return (
    <div className="flex flex-col p-6 bg-white rounded-3xl border border-slate-100 hover:border-blue-200 hover:shadow-lg transition-all cursor-pointer group">
      <div
        className={`w-10 h-10 rounded-xl mb-4 flex items-center justify-center ${colors[color]}`}
      >
        <Icon size={20} />
      </div>
