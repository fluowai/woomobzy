...
const SupportPortal: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'open':
        return (
          <span className="px-2 py-1 bg-emerald-100 text-emerald-700 text-[10px] font-bold uppercase rounded-md">
            Aberto
          </span>
        );
      case 'in_progress':
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Acompanhe seus chamados e tire dúvidas com nosso time.
            </p>
          </div>
        </div>
        <button
          ...
          {selectedTicket ? (
            <>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-50/30">
                ...
                {messages.map((msg) => (
                  <div
                    key={msg.id}
                    className={`flex gap-4 ${msg.is_admin_reply ? 'flex-row' : 'flex-row'}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-bold ${msg.is_admin_reply ? 'bg-amber-500 text-white' : 'bg-indigo-600 text-white'}`}
                    >
                      {msg.is_admin_reply
                        ? 'S'
                    ...
                    <div className="flex-1">
                      <div
                        className={`rounded-2xl p-4 shadow-sm border ${msg.is_admin_reply ? 'bg-amber-50 border-amber-100' : 'bg-white border-gray-100'}`}
                      >
                        {msg.is_admin_reply && (
                          <span className="text-[10px] font-bold uppercase text-amber-600 mb-1 block">
                            Equipe de Suporte
                          </span>
                        )}
                        <p className="text-sm text-gray-700 whitespace-pre-wrap">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center text-gray-400">
              ...
              <p className="text-sm max-w-xs mt-2">
                Selecione um chamado ao lado para ver o histórico de conversas.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl max-w-lg w-full overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="p-8 border-b border-gray-100 bg-gradient-to-r from-indigo-600 to-blue-600 text-white">
              <h3 className="text-2xl font-bold italic tracking-tighter uppercase mb-1">
                Novo Chamado
              </h3>
