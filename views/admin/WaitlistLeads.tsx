...
import { Mail, Phone, Clock, User, Download, Search, Filter } from 'lucide-react';
...
const WaitlistLeads: React.FC = () => {
  ...
  const handleExport = () => {
    const csvContent = "data:text/csv;charset=utf-8," 
      ...
      + filteredLeads.map(l => `${l.name},${l.email},${l.phone},${new Date(l.created_at).toLocaleDateString()}`).join("\n");
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-800 uppercase tracking-tight italic">
            Lista de <span className="text-orange-500">Espera e Pré-Lançamento</span>
          </h1>
          <p className="text-slate-500 text-sm">
            Pessoas interessadas que deixaram contatos enquanto seu site está em modo "Em Breve".
          </p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total de Interessados</p>
          <p className="text-3xl font-black text-slate-800">{leads.length}</p>
        </div>
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Novos (Últimos 7 dias)</p>
          <p className="text-3xl font-black text-orange-500">
            {leads.filter(l => new Date(l.created_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length}
          </p>
        </div>
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Página de Origem</p>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-50">
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Data de Inscrição</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr>
                  <td colSpan={3} className="px-8 py-12 text-center text-slate-400 italic">Carregando contatos...</td>
                </tr>
              ) : filteredLeads.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-8 py-12 text-center text-slate-400 italic">Nenhum interessado encontrado.</td>
                </tr>
              ) : (
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-orange-500 font-bold uppercase tracking-widest">Lead de Pré-Lançamento</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
