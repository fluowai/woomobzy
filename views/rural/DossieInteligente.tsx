    ...
    ? [
        {
          ...
          valid: Boolean(legal?.geo || legal?.geometry),
          icon: Map,
        },
        {
          label: 'CAR (Ambiental)',
          valid: Boolean(legal?.car || legal?.carNumber),
          icon: ShieldCheck,
        },
        {
          label: 'Matrícula',
          valid: Boolean(legal?.matricula),
          icon: FileSearch,
        },
      ]
    : [];
  ...
  const shareOnWhatsApp = () => {
    ...
    const message = [
      ...
      new Intl.NumberFormat('pt-BR', {
        style: 'currency',
        currency: 'BRL',
      }).format(selectedProperty.price || 0),
    ].join(' - ');
    window.open(
    ...
    <div className="space-y-10 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-black/60 font-medium italic">
            Análise técnica, jurídica e de mercado unificada em um único
            documento.
          </p>
        </div>
      ...
      {loading ? (
        <div className="py-32 text-center bg-white rounded-[3rem] border border-slate-100">
          <FileSearch
            size={64}
            className="text-slate-200 mx-auto mb-6 animate-pulse"
          />
          <h3 className="text-2xl font-bold text-black uppercase italic tracking-tighter">
            Carregando Ativos Rurais
          </h3>
          <p className="text-slate-400 font-medium italic mt-2">
            Buscando apenas propriedades rurais da sua organizacao.
          </p>
        </div>
      ) : !selectedProperty ? (
        <div className="py-32 text-center bg-white rounded-[3rem] border border-dashed border-slate-200">
          ...
          <p className="text-slate-400 font-medium italic mt-2">
            Escolha um imóvel para gerar o dossiê técnico 360.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="flex items-center gap-1 text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                      <Calendar size={12} /> Atualizado em:{' '}
                      {new Date(
                ...
                <div className="space-y-4">
                  {technicalItems.map((item, idx) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-[10px] font-bold uppercase ${item.valid ? 'text-emerald-600' : 'text-amber-600'}`}
                      >
                    ...
                    <p className="text-lg font-bold text-indigo-900">
                      {new Intl.NumberFormat('pt-BR', {
                        ...
                        (selectedProperty.price || 0) /
                          (selectedProperty.total_area_ha || 1)
                      )}
                    </p>
                    <p className="text-[9px] text-indigo-400 italic mt-1">
                      Calculado a partir do valor e da área cadastrados.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-amber-50 rounded-[2.5rem] p-10 border border-amber-100 relative overflow-hidden">
              <Zap
                size={48}
                className="absolute -bottom-4 -right-4 text-amber-200 opacity-20 rotate-12"
              />
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-bold uppercase italic tracking-tighter text-amber-900">
                    Análise de Risco IA
                  </h3>
                  <p className="text-[10px] font-bold text-amber-600 uppercase tracking-widest">
                    Processamento Neural 360
                  </p>
                </div>
              </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-black rounded-[3rem] p-10 text-white shadow-2xl shadow-slate-200">
              <h3 className="text-lg font-bold uppercase italic tracking-tighter mb-8">
                Ferramentas de Venda
              </h3>
              <div className="space-y-4">
                <button
                  onClick={shareOnWhatsApp}
                  className="w-full p-6 bg-white/5 border border-white/10 rounded-2xl hover:bg-emerald-600 hover:border-emerald-500 transition-all text-left group"
                >
