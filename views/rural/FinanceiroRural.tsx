...
const FinanceiroRural: React.FC = () => {
  ...
  const soldProperties = properties.filter((property) => {
    ...
    const soldAt = new Date(
      (property as any).updated_at || (property as any).created_at || 0
    );
    return (
      soldAt.getMonth() === currentMonth && soldAt.getFullYear() === currentYear
    );
  });
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex bg-white p-2 rounded-2xl border border-slate-100 shadow-sm">
          <div className="px-4 py-2 border-r border-slate-100">
            ...
            <input
              ...
              onChange={(event) =>
                setGoals((current) => ({
                  ...current,
                  monthly_vgv: Number(event.target.value),
                }))
              }
          ...
          <button
            onClick={saveGoal}
            disabled={savingGoal}
            title="Salvar metas rurais"
            className="p-3 text-indigo-600 disabled:opacity-50"
          >
            <Save size={20} />
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          {
            label: 'VGV Acumulado',
            value: totalVgv.toLocaleString('pt-BR', {
              style: 'currency',
              currency: 'BRL',
            }),
            icon: TrendingUp,
          ...
          },
          {
            ...
            value: estimatedCommission.toLocaleString('pt-BR', {
              style: 'currency',
              currency: 'BRL',
            }),
            icon: Award,
          ...
          {
            label: 'Ticket Médio',
            value: (soldProperties.length > 0
              ? totalVgv / soldProperties.length
              : 0
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-slate-400 text-sm">
                  Volume de negócios em relação à meta do período.
                </p>
              </div>
              ...
              <div>
                <div className="flex justify-between mb-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest text-indigo-400">
                    R$ {totalVgv.toLocaleString()} / R${' '}
                    {goals.monthly_vgv.toLocaleString()}
                  </span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-8 pt-4">
                <div className="p-6 bg-white/5 rounded-3xl border border-white/10">
                  ...
                  <p className="text-[10px] text-emerald-400 font-bold mt-1">
                    Potencial:{' '}
                    {proposalPotential.toLocaleString('pt-BR', {
                      style: 'currency',
                      currency: 'BRL',
                    })}
                  </p>
                ...
                <div className="p-6 bg-white/5 rounded-3xl border border-white/10">
                  ...
                  <p className="text-2xl font-bold">
                    {leads.filter((l) => l.status === 'Visita').length} Clientes
                  </p>
                  <p className="text-[10px] text-indigo-400 font-bold mt-1">
                    Participação no funil: {visitConversion.toFixed(1)}%
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
          <h4 className="text-xl font-bold uppercase italic tracking-tighter text-slate-900 mb-6">
            Funil de Vendas Rural
          </h4>
          <div className="space-y-4">
            {[
              ...
              },
              {
                ...
                count: leads.filter((l) => l.status === 'Em Atendimento')
                  .length,
                color: 'bg-amber-500',
                width: 'w-4/5',
              },
