...
const PortalProprietarioRural: React.FC = () => {
    ...
    const load = async () => {
      ...
      const [propertyResult, documentResult, leadResult] = await Promise.all([
        supabase
          .from('properties')
          .select(
            'id,title,price,status,city,state,total_area_ha,features,niche,property_type,created_at'
          )
        ...
        supabase
          .from('documents')
          .select(
            'id,property_id,original_name,document_type,status,validation_status,created_at'
          )
          .eq('organization_id', profile.organization_id)
          .order('created_at', { ascending: false }),
        supabase
      ...
      if (propertyResult.error)
        toast.error('Não foi possível carregar as propriedades rurais.');
      const ruralProperties = (
        (propertyResult.data || []) as RuralProperty[]
      ).filter(isRuralProperty);
      const ruralIds = new Set(ruralProperties.map((property) => property.id));
      setProperties(ruralProperties);
      setDocuments(
        ((documentResult.data || []) as RuralDocument[]).filter((document) =>
          ruralIds.has(document.property_id)
        )
      );
      setLeads(
  ...
  const proposals = leads.filter((lead) =>
    ['Proposta', 'Negociação'].includes(lead.status)
  ).length;
  const documentMap = useMemo(() => {
    const map = new Map<string, RuralDocument[]>();
    documents.forEach((document) => {
      map.set(document.property_id, [
        ...(map.get(document.property_id) || []),
        document,
      ]);
    });
    return map;
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="font-medium text-black/60">
          Acompanhamento real da carteira, documentação e oportunidades rurais.
        </p>
      </div>
        ...
        <div className="space-y-3 rounded-2xl border border-slate-200 bg-white p-6">
          {documents.length === 0 ? (
            <p className="py-10 text-center text-slate-400">
              Nenhum documento rural enviado.
            </p>
          ) : (
            documents.map((document) => {
              ...
              const property = properties.find(
                (item) => item.id === document.property_id
              );
              return (
                <div
                  key={document.id}
                ...
                >
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500">
                        {property?.title || 'Propriedade rural'} ·{' '}
                  ...
                  <span
                    className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${validated ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}
                  >
