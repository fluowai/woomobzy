...
const ValuationRural: React.FC = () => {
  ...
  const runValuationByCar = async (overrideCar?: string) => {
    ...
    try {
      ...
      toast.success(
        result.mode === 'created'
          ? 'Imovel criado e relatorio salvo.'
          : 'Relatorio CAR atualizado.'
      );
    } catch (error: any) {
  ...
  const refreshCurrent = async () => {
    const currentCar = firstText(
      enrichment.car_number,
      fullCar.codigo,
      legal.carNumber
    );
    if (!currentCar) {
  ...
  const downloadDossier = async () => {
    ...
    try {
      await downloadApiFile(
        `/api/rural/dossier/${selected.id}/pdf`,
        `valuation-rural-${selected.id}.pdf`
      );
    } catch (error: any) {
  ...
  const areaHa = Number(
    firstText(
      ...
      fullCar.area_ha,
      selected?.total_area_ha,
      features.areaHectares,
      0
    )
  );
  const carCode = firstText(
    enrichment.car_number,
    fullCar.codigo,
    legal.carNumber,
    'Pendente'
  ...
  const summaryCards = [
    {
      label: 'Valor medio',
      value: valuation.total_value_avg
        ? currency(valuation.total_value_avg)
        : 'Pendente',
      icon: TrendingUp,
    ...
    },
    {
      ...
      value: valuation.price_per_ha_avg
        ? currency(valuation.price_per_ha_avg)
        : 'Pendente',
      icon: BarChart3,
      tone: 'text-blue-700',
      bg: 'bg-blue-50',
    ...
    ? [
        {
          ...
          rows: [
            ...
            [
              'Municipio/UF',
              [municipality, uf].filter(Boolean).join(' / ') || 'Pendente',
            ],
          ...
          rows: [
            [
              ...
              enrichment.declared_area_ha
                ? number(enrichment.declared_area_ha, ' ha')
                : number(areaHa, ' ha'),
            ],
            [
              'Area medida',
              enrichment.measured_area_ha
                ? number(enrichment.measured_area_ha, ' ha')
                : number(areaHa, ' ha'),
            ],
            [
              'Centroide',
            ...
            [
              'Geometria',
              enrichment.geometry ? 'Poligono disponivel' : 'Pendente',
            ],
          ],
        },
        {
          ...
          rows: [
            ...
            [
              'Data-base',
              dateTime(valuation.valuation_date || enrichment.data_geracao),
            ],
            ['Status', valuation.status || 'Pendente'],
            [
              'Confianca',
              valuation.confidence_score !== undefined
                ? `${valuation.confidence_score}/100 (${confidenceLabel(valuation.confidence_score)})`
                : 'Pendente',
            ],
          ],
        },
          ...
          rows: [
            [
              ...
              valuation.total_value_min
                ? currency(valuation.total_value_min)
                : 'Pendente',
            ],
            [
              'Valor medio',
            ...
            [
              ...
              valuation.total_value_max
                ? currency(valuation.total_value_max)
                : 'Pendente',
            ],
            [
              'Comparaveis usados',
              valuation.comparable_count !== undefined
                ? `${valuation.comparable_count} registros internos, sem exibir fazendas`
                : 'Pendente',
            ],
          ],
        },
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
        <div>
          ...
          <p className="text-sm text-slate-500 max-w-3xl mt-2">
            Informe um CAR e gere uma tela unica com fontes territoriais,
            ambientais, economicas e pesquisa regional via Groq.
          </p>
        </div>
      ...
      {!selected ? (
        <section className="rounded-lg border border-dashed border-slate-200 bg-white p-12 text-center">
          ...
          <p className="mt-4 text-sm font-bold text-slate-500">
            Digite o CAR para gerar o relatorio completo do imovel.
          </p>
        </section>
      ) : (
        <>
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-1 text-2xl font-bold text-slate-950">
                  {selected.title || `CAR ${carCode}`}
                </h3>
                <p className="mt-2 text-sm font-semibold text-slate-500">
                  {[municipality, uf].filter(Boolean).join(' / ') ||
              ...
              <div className="rounded-lg bg-slate-950 px-4 py-3 text-right text-white">
                ...
                <p className="max-w-[360px] break-all text-xs font-bold">
                  {carCode}
                </p>
              </div>
            </div>
          ...
          <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {summaryCards.map((card) => (
              ...
              >
                <div
                  className={`mb-4 flex h-10 w-10 items-center justify-center rounded-lg ${card.bg} ${card.tone}`}
                >
                  <card.icon size={20} />
                </div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <h4 className="text-lg font-bold text-slate-950">
                  Laudo preliminar de valuation por CAR
                </h4>
              </div>
                ...
                >
                  ...
                  <div className="space-y-2">
                    {section.rows.map(([label, value]) => (
                      <div
                        key={label as string}
                        className="flex items-start justify-between gap-4 border-t border-slate-200 pt-2"
                      >
                        <span className="text-[11px] font-bold uppercase tracking-wide text-slate-400">
                          {label}
                        </span>
                        <span className="max-w-[62%] text-right text-xs font-bold text-slate-800">
            ...
            <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {sources.map(([label, source]: any) => {
                ...
                return (
                  <div
                    key={label}
                    className={`rounded-lg px-3 py-3 ${sourceTone(status)}`}
                  >
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-6">
            ...
            <p className="mt-4 text-sm font-bold text-slate-700">
              {valuation.regional_summary ||
                regional.market_summary ||
                'Analise regional ainda nao gerada.'}
            </p>
              ...
              {regionalCards.map(([label, item]: any) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm font-bold text-slate-900">
                    {item?.summary || 'Sem detalhe disponivel.'}
                  </p>
                  <p className="mt-2 text-[11px] font-bold uppercase tracking-wide text-slate-500">
                    Confianca {item?.confidence || 'baixa'} |{' '}
                    {item?.source || 'Fonte regional'}
                  </p>
                </div>
              ))}
            </div>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="rounded-lg border border-emerald-100 bg-emerald-50 p-6">
              <h4 className="text-sm font-bold uppercase tracking-widest text-emerald-900">
                Pontos Que Ajudam
              </h4>
              <div className="mt-4 space-y-2">
                {(
                  valuation.drivers || [
                    'Rode a consulta para gerar os pontos positivos.',
                  ]
                ).map((item: string) => (
                  <div
                    key={item}
                    className="flex items-start gap-2 text-sm font-semibold text-emerald-900"
                  >
            ...
            <div className="rounded-lg border border-amber-100 bg-amber-50 p-6">
              <h4 className="text-sm font-bold uppercase tracking-widest text-amber-900">
                Pendencias Para Melhorar
              </h4>
              <div className="mt-4 space-y-2">
                {(
                  valuation.risks || [
                    'Rode a consulta para gerar as pendencias.',
                  ]
                ).map((item: string) => (
                  <div
                    key={item}
          ...
          {(ambiental.prodes ||
            ambiental.deter ||
            ambiental.mapbiomas ||
            economico.producao_agricola) && (
            <section className="rounded-lg border border-slate-200 bg-white p-6">
              <h4 className="text-sm font-bold uppercase tracking-widest text-slate-950">
                Dados complementares consultados
              </h4>
                ...
                <div className="rounded-lg bg-slate-50 p-4">
                  ...
                  <p className="mt-2 text-sm font-bold text-slate-900">
                    {ambiental.prodes?.possui_desmatamento === false
                      ? 'Sem desmatamento detectado'
                      : ambiental.prodes?.erro || 'Consulta executada'}
                  </p>
                </div>
                <div className="rounded-lg bg-slate-50 p-4">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                  ...
                  <p className="mt-2 text-sm font-bold text-slate-900">
                    {ambiental.mapbiomas?.total_alertas !== undefined
                      ? `${ambiental.mapbiomas.total_alertas} alerta(s)`
                      : ambiental.mapbiomas?.mensagem || 'Consulta executada'}
                  </p>
                </div>
                <div className="rounded-lg bg-slate-50 p-4">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                  ...
                  <p className="mt-2 text-sm font-bold text-slate-900">
                    {economico.producao_agricola?.produtos_principais?.[0]
                      ?.produto || 'Dados municipais consultados'}
                  </p>
                </div>
              </div>
            </section>
          ...
          {lastResult?.car?.number && (
            <p className="text-xs font-semibold text-slate-400">
              Ultima consulta: {lastResult.car.number} |{' '}
