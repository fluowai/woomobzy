...
const ExportadorPortais: React.FC = () => {
  ...
  const load = async () => {
    ...
    const [{ data: integrations }, { data: syncLogs }] = await Promise.all([
      ...
      supabase
        ...
        .select(
          'id,status,message,created_at,integration:integration_id(portal_name)'
        )
        .eq('organization_id', profile.organization_id)
    ...
    const byKey = new Map(
      (integrations || []).map((item: any) => [item.portal_key, item])
    );
    setPortals(
      DEFAULT_PORTALS.map((portal) => {
        ...
        return saved
          ? {
              ...
              lastSync: saved.last_sync_at
                ? new Date(saved.last_sync_at).toLocaleString('pt-BR')
                : '-',
  ...
  const toggleConnection = async (portal: Portal) => {
    ...
    const feedUrl = enabled
      ? `${window.location.origin}/api/portals/${portal.key}/feed.xml`
      : '';
    await supabase.from('urban_portal_integrations').upsert(
      {
  ...
  const totalExported = connected.reduce(
    (sum, portal) => sum + portal.properties,
    0
  );
  const successRate =
    connected.length > 0
      ? `${Math.round(((connected.length - connected.reduce((sum, p) => sum + p.errors, 0)) / connected.length) * 100)}%`
      : '-';
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="font-medium text-black/60">
          Feed XML automatico para portais imobiliarios urbanos.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
        {stats.map((stat) => (
          ...
          >
            <div
              className={`mb-4 w-fit rounded-2xl p-3 ${stat.bg} ${stat.color}`}
            >
              <stat.icon size={24} />
            </div>
            <h3 className="mb-1 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">
              {stat.label}
            </h3>
            <p className="text-2xl font-bold italic tracking-tighter text-slate-900">
              {stat.value}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {portals.map((portal) => (
          <div
            key={portal.key}
            className={`rounded-2xl border-2 bg-white transition-all ${portal.connected ? 'border-blue-200 shadow-lg' : 'border-slate-100'}`}
          >
            <div className="p-6">
                ...
                <div className="space-y-3 border-t border-slate-100 pt-4">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-1">
                      <code className="max-w-[150px] truncate rounded bg-slate-50 px-2 py-1 text-xs text-blue-600">
                        {portal.feedUrl}
                      </code>
                      <button
                        onClick={() =>
                  ...
                  <div className="flex gap-2 pt-2">
                    <button
                      onClick={() => registerSync(portal)}
                      className="flex flex-1 items-center justify-center gap-1 rounded-xl bg-blue-50 p-2 text-xs font-medium text-blue-600 transition-all hover:bg-blue-100"
                    >
                      <RefreshCw size={14} /> Sincronizar
                    </button>
                    <button
                      onClick={() => setShowConfig(portal.id)}
                      className="flex flex-1 items-center justify-center gap-1 rounded-xl bg-slate-50 p-2 text-xs font-medium text-slate-600 transition-all hover:bg-slate-100"
                    >
              ...
              >
                <div>
                  <p className="text-sm font-medium text-black">
                    {log.integration?.portal_name || 'Portal'}
                  </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl bg-white p-8 shadow-2xl">
            ...
            <p className="text-sm text-slate-500">
              As credenciais especificas de cada portal podem ser ligadas aqui
              na proxima etapa de integracao oficial.
            </p>
            <button
              onClick={() => setShowConfig(null)}
              className="mt-6 w-full rounded-xl bg-blue-600 py-3 text-sm font-bold text-white shadow-lg transition-all hover:bg-blue-500"
            >
