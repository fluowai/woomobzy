...
import { AlertCircle, BarChart2, CheckCircle, CreditCard, DollarSign, Send, TrendingDown, TrendingUp } from 'lucide-react';
import { cobrancaService, type Billing, type DashboardFinanceiro } from '../../services/cobrancaService';
...
export default function FinanceiroUrbano() {
  ...
  const resumo = useMemo(
    () => [
      {
        label: 'A Receber (Mes)',
        value: cobrancaService.formatCurrency(dashboard?.totais?.total_aberto || 0),
        icon: TrendingUp,
      ...
      {
        label: 'Recebido (Ano)',
        value: cobrancaService.formatCurrency(dashboard?.totais?.total_recebido_ano || 0),
        icon: CheckCircle,
        color: 'text-blue-600',
        bg: 'bg-blue-50',
      ...
      {
        label: 'Inadimplencia',
        value: cobrancaService.formatCurrency(dashboard?.totais?.total_vencido || 0),
        icon: AlertCircle,
      ...
      },
      {
        ...
        value: cobrancaService.formatCurrency(dashboard?.totais?.receita_mensal_projetada || 0),
        icon: DollarSign,
        color: 'text-primary',
        bg: 'bg-primary/10',
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div>
        ...
        <p className="body mt-1 text-slate-500">Cobranca, fluxo de caixa, repasses e gestao de inadimplencia.</p>
      </div>
        ...
        {[
          ...
          <button
            ...
            className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-bold transition-colors ${
              tab === item.id ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
          >
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {['Cliente', 'Imovel', 'Vencimento', 'Valor', 'Status', 'Acao'].map((header) => (
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr><td colSpan={6} className="p-10 text-center text-sm text-slate-400">Carregando financeiro...</td></tr>
              ) : billings.length === 0 ? (
                <tr><td colSpan={6} className="p-10 text-center text-sm text-slate-400">Nenhuma cobranca encontrada.</td></tr>
              ) : (
                billings.slice(0, 20).map((billing) => (
                  <tr key={billing.id} className="transition-colors hover:bg-slate-50/50">
                    <td className="p-4 text-sm font-bold text-slate-900">{billing.contract?.tenant_name || 'Cliente'}</td>
                    <td className="p-4 text-sm text-slate-600">{billing.contract?.property?.title || '-'}</td>
                    <td className="p-4 text-sm text-slate-600">{billing.due_date ? new Date(billing.due_date).toLocaleDateString('pt-BR') : '-'}</td>
                    <td className="p-4 font-bold text-slate-900">{cobrancaService.formatCurrency(billing.amount || 0)}</td>
                    <td className="p-4">
                      <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ${statusColors[billing.status || 'aberto'] || 'bg-slate-100 text-slate-600'}`}>
                        {billing.status || 'aberto'}
                      </span>
                    </td>
                    <td className="p-4">
                      {billing.status === 'pago' ? (
                        <span className="flex items-center gap-1 text-xs font-bold text-green-600"><CheckCircle size={14} /> Quitado</span>
                      ) : (
