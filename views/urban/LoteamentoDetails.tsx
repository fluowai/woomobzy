  ...
  if (loading)
    return (
      <div className="p-10 text-center animate-pulse">
        Carregando Espelho de Vendas...
      </div>
    );
  if (!development)
    return <div className="p-10 text-center">Loteamento não encontrado.</div>;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-blue-600 text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/20"
          >
          ...
          <div className="flex-1 overflow-y-auto p-6 lg:p-10 custom-scrollbar">
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-12 gap-3 lg:gap-4">
              {filteredLots.map((lot) => (
                ...
                >
                  <span className="text-[10px] font-bold opacity-60 leading-none uppercase">
                    {lot.block_id.split(' ')[1]}
                  </span>
                  <span className="text-xl font-bold italic tracking-tighter leading-none">
                    {lot.number}
                  </span>
                </button>
              ))}
            ...
            {filteredLots.length === 0 && (
              <div className="rounded-[2rem] border-2 border-dashed border-slate-200 bg-white p-10 text-center">
                ...
                <p className="mt-2 text-xs text-slate-400">
                  Clique em Novo Lote para iniciar o espelho de vendas real
                  deste loteamento.
                </p>
              </div>
            )}
          ...
          {selectedLot ? (
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center ${getStatusColor(selectedLot.status)} shadow-lg`}
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  <p className="text-lg font-bold text-slate-900">
                    {selectedLot.area_m2.toFixed(2)} m²
                  </p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">
            ...
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-lg font-bold text-slate-900 uppercase italic tracking-tighter">
                Selecione um Lote
              </h3>
              <p className="text-xs text-slate-400 mt-2 max-w-[200px]">
                Clique em um lote no mapa ao lado para ver detalhes e realizar
                ações.
              </p>
            </div>
          )}
