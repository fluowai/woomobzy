  ...
  const load = async () => {
    ...
    const [{ data: propertyData }, { data: favoriteData }] = await Promise.all([
      supabase
        .from('properties')
        .select(
          'id,title,property_type,price,purpose,city,state,neighborhood,images,features,niche'
        )
    ...
    setProperties(
      ((propertyData || []) as UrbanProperty[]).filter(isUrbanProperty)
    );
    setFavoriteIds(
      new Set((favoriteData || []).map((item) => item.property_id))
    );
    setLoading(false);
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="font-medium text-slate-500">
          Consulte o estoque urbano publicado pela imobiliaria.
        </p>
      </div>
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`rounded-xl px-5 py-3 text-sm font-bold ${activeTab === 'buscar' ? 'bg-blue-600 text-white' : 'border bg-white text-slate-500'}`}
        >
          Buscar imoveis
        </button>
        <button
          onClick={() => setActiveTab('favoritos')}
          className={`rounded-xl px-5 py-3 text-sm font-bold ${activeTab === 'favoritos' ? 'bg-blue-600 text-white' : 'border bg-white text-slate-500'}`}
        >
      ...
      {loading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((property) => {
            ...
            const area = Number(
              features.areaM2 || features.areaConstruida || 0
            );
            return (
              <article
                key={property.id}
              ...
              >
                <div className="relative h-44 bg-slate-100">
                  ...
                  >
                    <Heart
                      size={17}
                      fill={
                        favoriteIds.has(property.id) ? 'currentColor' : 'none'
                      }
                    />
                  </button>
                </div>
                <div className="p-5">
                  ...
                  <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-500">
                    {features.dormitorios > 0 && (
                      <span className="flex items-center gap-1">
                        <Bed size={15} /> {features.dormitorios}
                      </span>
                    )}
                    {features.banheiros > 0 && (
                      <span className="flex items-center gap-1">
                        <Bath size={15} /> {features.banheiros}
                      </span>
                    )}
                    {features.vagas > 0 && (
                      <span className="flex items-center gap-1">
                        <Car size={15} /> {features.vagas}
                      </span>
                    )}
                    {area > 0 && (
                      <span className="flex items-center gap-1">
                        <Maximize size={15} /> {area} m2
                      </span>
                    )}
                  </div>
