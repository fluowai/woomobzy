      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-black transition-all shadow-lg shadow-slate-900/10"
          >
          ...
          <button
            ...
            disabled={saving}
            className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-blue-700 disabled:opacity-50 transition-all shadow-lg shadow-blue-500/20"
          >
        ...
        <div className="lg:col-span-8 bg-white rounded-[2.5rem] border border-slate-100 shadow-xl p-8 lg:p-12 space-y-10">
          <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <label className="text-[10px] font-bold uppercase text-slate-400 tracking-[0.2em] ml-2">
          ...
          <section className="space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h3 className="text-xs font-bold text-slate-900 uppercase tracking-widest flex items-center gap-2">
                <RefreshCw size={16} className="text-blue-600" /> Balões /
                Reforços
              </h3>
              <button
                onClick={addBalloon}
            ...
            {balloons.length === 0 ? (
              <div className="py-10 text-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                <p className="text-sm font-bold text-slate-400 italic">
                  Nenhum balão programado. Adicione reforços anuais ou
                  semestrais.
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex-1 grid grid-cols-2 gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          value={b.month}
                          onChange={(e) =>
                            updateBalloon(b.id, 'month', Number(e.target.value))
                          }
        ...
        <aside className="lg:col-span-4 space-y-6">
          <div className="bg-slate-900 text-white rounded-[2.5rem] shadow-2xl p-10 space-y-10 sticky top-10">
            ...
            <div className="space-y-6 border-t border-white/10 pt-8">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-bold text-amber-400">
                  {formatCurrency(
                    balloons.reduce((acc, b) => acc + b.amount, 0)
                  )}
                </span>
              </div>
              <div className="flex justify-between items-center">
            ...
            <section className="space-y-4">
              <div className="flex items-center gap-3 text-white/60">
                ...
                <p className="text-xs font-medium">
                  Correção sugerida: <strong>IPCA + 0,5%</strong>
                </p>
              </div>
              <div className="flex items-center gap-3 text-white/60">
                <Calculator size={18} className="text-emerald-400" />
                <p className="text-xs font-medium">
                  Tabela: <strong>PRICE (Amortização Constante)</strong>
                </p>
              </div>
            </section>
            ...
            <button
              ...
              disabled={saving}
              className="w-full py-5 bg-blue-600 text-white rounded-2xl font-bold uppercase text-xs tracking-[0.2em] shadow-xl shadow-blue-500/20 hover:scale-[1.02] active:scale-95 disabled:opacity-50 transition-all"
            >
