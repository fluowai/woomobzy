...
const BANKS: BankCondition[] = [
  { id: 'caixa', name: 'Caixa Econômica', logoInitial: 'C', color: 'bg-emerald-100 text-emerald-700', taxa: 10.49, cet: 11.21 },
  { id: 'itau', name: 'Itaú', logoInitial: 'I', color: 'bg-orange-100 text-orange-700', taxa: 10.89, cet: 11.67 },
  { id: 'bradesco', name: 'Bradesco', logoInitial: 'B', color: 'bg-red-100 text-red-700', taxa: 11.12, cet: 11.94 },
];
...
export default function Simulator360() {
  ...
  const calculateSimulation = (bank: BankCondition, sys: 'SAC' | 'PRICE') => {
    ...
    if (sys === 'SAC') {
      ...
      const installment = (financedAmount * (monthlyRate * Math.pow(1 + monthlyRate, termMonths))) / (Math.pow(1 + monthlyRate, termMonths) - 1);
  ...
  const saveSimulation = async (status: 'draft' | 'proposal' = 'draft') => {
    ...
    if (error) {
      ...
      setFeedback(status === 'proposal' ? 'Proposta registrada!' : 'Simulação salva!');
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto pb-12 font-sans text-slate-800 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Simulador de financiamento</h1>
          <p className="text-sm text-slate-500 mt-1">Compare condições e crie a melhor proposta para seu cliente</p>
        </div>
        <div className="flex items-center gap-2 bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100">
          <CheckCircle2 size={16} className="text-emerald-600" />
          <span className="text-xs font-bold text-emerald-700">Taxas atualizadas hoje</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center mb-10 max-w-2xl mx-auto w-full relative">
        ...
        <div className="flex justify-between w-full">
          <div className="flex items-center gap-2 bg-[#f8fafc] px-2">
            <div className="w-6 h-6 rounded-full bg-emerald-600 text-white flex items-center justify-center text-xs font-bold shadow-sm">1</div>
            <span className="text-sm font-bold text-slate-900">Dados do imóvel</span>
          </div>
          <div className="flex items-center gap-2 bg-[#f8fafc] px-2">
            <div className="w-6 h-6 rounded-full bg-slate-200 text-slate-500 flex items-center justify-center text-xs font-bold">2</div>
            <span className="text-sm font-medium text-slate-500">Condições</span>
          </div>
          <div className="flex items-center gap-2 bg-[#f8fafc] px-2">
            <div className="w-6 h-6 rounded-full bg-slate-200 text-slate-500 flex items-center justify-center text-xs font-bold">3</div>
            <span className="text-sm font-medium text-slate-500">Comparar bancos</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 lg:p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Valor do imóvel</label>
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Entrada</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 font-bold text-slate-400">R$</span>
                  <input
                    type="number"
                ...
                <div className="mt-4 flex items-center gap-3">
                  ...
                  <input 
                    ...
                    onChange={e => handleEntryPercentageChange(Number(e.target.value))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Prazo</label>
                <div className="relative">
                  <select
                    value={termYears}
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Sistema de amortização</label>
          ...
          {/* Banks Table */}
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 lg:p-8">
            <h3 className="text-base font-bold text-slate-900 mb-6">Melhores condições encontradas</h3>
            
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="pb-3 text-xs font-medium text-slate-400">Banco</th>
                    <th className="pb-3 text-xs font-medium text-slate-400">Taxa a.a.</th>
                    <th className="pb-3 text-xs font-medium text-slate-400">Parcela inicial</th>
                    <th className="pb-3 text-xs font-medium text-slate-400">CET</th>
                    <th className="pb-3 text-xs font-medium text-slate-400">Total</th>
                    <th className="pb-3 text-xs font-medium text-slate-400">Ação</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {BANKS.map((bank, index) => {
                    ...
                    return (
                      <tr key={bank.id} className={`transition-colors cursor-pointer hover:bg-slate-50/50 ${isSelected ? 'bg-emerald-50/30' : ''}`} onClick={() => setSelectedBankId(bank.id)}>
                        <td className="py-4">
                          <div className="flex items-center gap-3">
                            <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${isSelected ? 'border-emerald-600 bg-white' : 'border-slate-300'}`}>
                              {isSelected && <div className="w-2.5 h-2.5 bg-emerald-600 rounded-full" />}
                            </div>
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${bank.color}`}>
                              {bank.logoInitial}
                            </div>
                            <div>
                              <p className="text-sm font-bold text-slate-900">{bank.name}</p>
                              {isBest && <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded">Melhor opção</span>}
                            </div>
                          </div>
                        </td>
                        <td className="py-4">
                          <p className="text-sm font-medium text-slate-700">{bank.taxa.toFixed(2).replace('.', ',')}%</p>
                        </td>
                        <td className="py-4">
                          <p className="text-sm font-bold text-slate-900">{formatCurrency(sim.firstInstallment)}</p>
                        </td>
                        <td className="py-4">
                          <p className="text-sm font-medium text-slate-700">{bank.cet.toFixed(2).replace('.', ',')}%</p>
                        </td>
                        <td className="py-4">
                          <p className="text-sm font-medium text-slate-700">{formatCompactCurrency(sim.totalCost)}</p>
                        </td>
                        <td className="py-4">
            ...
            <p className="text-[10px] text-slate-400 mt-6 flex items-center gap-1.5">
              Condições válidas para perfil padrão. Sujeitas à análise de crédito.
            </p>
          </div>
        ...
        <div className="w-full xl:w-96 space-y-6 shrink-0">
          <div className="bg-[#f8fafc] border border-slate-200 rounded-2xl shadow-sm p-6 lg:p-8">
            <h3 className="text-base font-bold text-slate-900 mb-6">Resumo da simulação</h3>
            
            <div className="flex items-center gap-3 mb-6">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${selectedBank.color}`}>
                {selectedBank.logoInitial}
              </div>
              <p className="text-sm font-medium text-slate-700">{selectedBank.name} • {amortization}</p>
            </div>

            <div className="mb-8">
              <p className="text-xs font-medium text-slate-500 mb-1">Primeira parcela</p>
            ...
            <div className="space-y-3 mb-8">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-bold text-slate-900">{formatCurrency(entryValue)}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-bold text-slate-900">{formatCurrency(simResult.lastInstallment)}</span>
              </div>
              <div className="flex justify-between items-center text-sm border-t border-slate-200 pt-3">
                ...
                <span className="font-bold text-slate-900">{formatCurrency(simResult.totalCost)}</span>
              </div>
            </div>

            <div className="mb-8">
              <h4 className="text-xs font-medium text-slate-500 mb-4">Evolução das parcelas ({amortization})</h4>
            ...
            <div className="space-y-3">
              ...
              {feedback && (
                <p className="text-center text-xs font-bold text-emerald-600 mt-2">{feedback}</p>
              )}
            </div>

            <p className="text-[10px] text-center text-slate-400 mt-4 flex items-center justify-center gap-1">
              Simulação indicativa • Consulte condições do banco <Info size={12} />
            </p>
          </div>
        </div>
