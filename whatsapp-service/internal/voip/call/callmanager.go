...
import (
	...
	"whatsapp-service/internal/voip/transport"
...
func NewCallManager(sock core.VoipSocket, log *zap.SugaredLogger) *CallManager {
	if log == nil {
...
func (m *CallManager) StartCall(ctx context.Context, callID string, peerJid types.JID, isVideo bool) error {
	...
	m.log.Infow("call offer sent", "call_id", callID, "peer", resolved.String())
	return nil
}
...
func (m *CallManager) AcceptCall(ctx context.Context, callID string) error {
	...
	if key != nil {
		...
		if err != nil {
			m.log.Errorw("build accept failed", "err", err)
		} else if err := m.sock.SendNode(ctx, acceptNode); err != nil {
			m.log.Errorw("accept send error", "err", err)
		}
	}
	...
	if relayData != nil {
		...
		m.log.Warnw("call accepted but no relay endpoints yet; media path waits for a transport message", "call_id", callID)
	}
	m.log.Infow("call accepted", "call_id", callID)
	return nil
}
