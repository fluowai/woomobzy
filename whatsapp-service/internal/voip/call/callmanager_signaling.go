...
import (
	...
	"whatsapp-service/internal/voip/wanode"
...
func (m *CallManager) HandleCallOffer(ctx context.Context, node *waBinary.Node, peerJid types.JID) {
	...
	if err != nil {
		m.log.Errorw("offer decrypt call key", "err", err)
	}
	...
	if len(relays) == 0 {
		...
		if parsed := signaling.ParseRelayFromAck(info.InnerNode); len(parsed.Relays) > 0 {
			...
			structured = &parsed
			m.log.Infow("offer relays parsed via structured (te2) format", "call_id", callID, "relays", len(relays))
		}
	}
	...
	m.log.Debugw("offer inner node structure", "call_id", callID, "children", childTagSummary(info.InnerNode))
	...
	if err := m.sock.SendNode(ctx, preaccept); err != nil {
		m.log.Errorw("send preaccept", "err", err)
	}
	...
	m.emitState()
	m.mu.Unlock()
	m.log.Infow("incoming call", "call_id", callID, "peer", peerJid.String(), "video", isVideo, "relays", len(relays))
}
...
func (m *CallManager) HandleCallAccept(ctx context.Context, node *waBinary.Node, peerJid types.JID) {
	...
	m.log.Infow("remote accepted call", "call_id", call.CallID, "peer", peerJid.String(),
	...
	if hasConn {
		...
		if err := call.ApplyTransition(Transition{Type: TransitionMediaConnected}); err == nil {
			...
			m.log.Infow("call ACTIVE (media path established)", "call_id", call.CallID, "audio", m.codec != nil)
		}
		m.mu.Unlock()
...
func (m *CallManager) HandleCallTransport(ctx context.Context, node *waBinary.Node, peerJid types.JID) {
	...
	relays := signaling.ExtractRelayEndpoints(info.InnerNode)
	m.log.Infow("call transport received", "call_id", call.CallID,
...
func (m *CallManager) HandleCallAck(ctx context.Context, node *waBinary.Node) {
	...
	if e := wanode.AttrString(node.Attrs, "error"); e != "" {
		m.log.Errorw("offer ack error", "error", e)
		return
	}
	parsed := signaling.ParseRelayFromAck(node)
	m.log.Infow("offer ack received", "relays", len(parsed.Relays), "participants", len(parsed.ParticipantJids))
...
func (m *CallManager) HandleCallTerminate(node *waBinary.Node) {
	...
	m.log.Infow("call terminated by peer", "call_id", call.CallID, "reason", string(reason))
