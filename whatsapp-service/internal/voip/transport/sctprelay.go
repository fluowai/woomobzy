...
func NewSctpRelayManager(log *zap.SugaredLogger) *SctpRelayManager {
	if log == nil {
...
func (m *SctpRelayManager) connectToRelay(info RelayConfig) {
	...
	m.log.Infow("relay connecting", "id", id, "name", info.Name)
	...
	if err != nil {
		m.log.Errorw("relay peerconnection failed", "id", id, "err", err)
	...
	pc.OnICEConnectionStateChange(func(s webrtc.ICEConnectionState) {
		m.log.Infow("relay ice state", "id", id, "state", s.String())
	...
	if err != nil {
		m.log.Errorw("relay datachannel failed", "id", id, "err", err)
		m.failConnection(conn)
		return
	}
	...
	channel.OnOpen(func() {
		m.log.Infow("relay datachannel open", "id", id)
	...
	if err := pc.SetRemoteDescription(webrtc.SessionDescription{Type: webrtc.SDPTypeAnswer, SDP: munged}); err != nil {
		m.log.Errorw("relay set remote description failed", "id", id, "err", err)
		m.failConnection(conn)
		return
	}
	...
	go func() {
		...
		case <-time.After(relayConnectionTimeout):
			if conn.state == relayStateConnecting {
				m.log.Debugw("relay connection timeout", "id", id)
				m.failConnection(conn)
			}
		case <-conn.stopCh:
		}
...
func (m *SctpRelayManager) sendRaw(conn *relayConnection, data []byte) {
	...
	if err := conn.channel.Send(data); err != nil {
		m.log.Debugw("relay send error", "id", conn.id, "err", err)
	}
}
